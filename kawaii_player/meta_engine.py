    ...
    def map_episodes(
        ...
        for i, val in enumerate(epn_arr_list):
            ...
            if key_found:
                ...
                summary = "Air Date: {}\n\n{}: {}\n\n{}".format(
                    ep_val[-3], ep_val[1], ep_val[3], ep_val[-1]
                )
                img_url = ep_val[-2]
                if extra:
                ...
                if img_url and img_url.startswith("http"):
                    ui.vnt.get(
                        ...
                        out=dest_picn,
                        onfinished=partial(
                            self.finished_thumbnails, i, new_name, summary, dest_picn
                        ),
                    )
            else:
                new_val = val
        ...
        if not name_srch:
            ...
            if not name_srch:
                ...
                if not name_srch:
                    ...
                    if ssn_final:
                        name_srch_val = (
                            ssn_final + "ep" + re.sub("-[^0-9]*", "", name_srch.group())
                        )
                    else:
                        oped = re.search("op[0-9]*|ed[0-9]*", lower_case)
                        if not oped:
                            name_srch_val = "ep" + re.sub(
                                "-[^0-9]*", "", name_srch.group()
                            )
    ...
    def getTvdbEpnInfo(
        ...
        for i in range(4, len(link), 4):
            ...
            if p:
                ...
                q = (
                    "http://thetvdb.com/banners/episodes/"
                    + series_id
                    + "/"
                    + poster_id
                    + ".jpg"
        ...
        for i, val in enumerate(epn_arr_list):
            ...
            if key_found:
                ...
                image_dict.update(
                    {new_name: [ep_val[2], ep_val[3], ep_val[4], extra, site]}
                )
                if extra:
                    new_val = new_name + "\t" + extra
                else:

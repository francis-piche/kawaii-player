...
def start_player_remotely(nm, mode):
    ...
    if mode == "normal" and ui.list2.count():
        ...
        ui.mpvplayer_val.processId() > 0 or ui.player_val == "libmpv"
    ) or ui.web_control == "slave":
        if mode == "stop":
    ...
    if shuffle:
        ...
        if st.lower() == "video":
            ...
            if (
                st_o.lower() == "artist"
                or st_o.lower() == "album"
                or st_o.lower() == "directory"
                or st_o.lower() == "playlist"
            ):
                for i in range(0, ui.list3.count()):
                    item = ui.list3.item(i)
                    txt = item.text()
            ...
            if srch:
                ...
                if len(list_item) > 0:
                    for i in list_item:
                        ...
                        if item:
                            ui.list1.itemDoubleClicked["QListWidgetItem*"].emit(item)
                            ui.gui_signals.click_title_list(
                                "playlist_click_from_client"
                            )
                            time.sleep(0.5)
                            if (
                                ui.instant_cast_play in range(0, ui.list2.count())
                                and srch.lower() == "tmp_playlist"
                            ):
                                ui.list2.setCurrentRow(ui.instant_cast_play)
                                item = ui.list2.item(ui.instant_cast_play)
                                if item:
                                    ui.list2.itemDoubleClicked["QListWidgetItem*"].emit(
                                        item
                                    )
            ...
            if index_val:
                ...
                if (
                    len(list_item) > 0
                    and ui.list3.currentItem().text().lower() != "history"
                ):
                    for i in list_item:
                        row = ui.list3.row(i)
                        ui.list3.setFocus()
    ...
    if mode == "video":
        ...
        if not os.path.exists(video_db):
            ui.media_data.create_update_video_db(
                video_db, video_file, video_file_bak, update_progress_show=False
            )
        else:
            ui.media_data.update_on_start_video_db(
                video_db,
                video_file,
                video_file_bak,
                "Update",
        ...
        if not os.path.exists(music_db):
            ui.media_data.create_update_music_db(
                music_db, music_file, music_file_bak, update_progress_show=False
            )
        else:
            ui.media_data.update_on_start_music_db(
                music_db, music_file, music_file_bak, update_progress_show=False
            )
...
class HTTPServer_RequestHandler(BaseHTTPRequestHandler):
            ...
            try:
                ...
                if nm.split("&")[4] == "True":
                    ...
                    if ui.https_media_server:
                        ...
                        https_val = "http"
                    nm = (
                        https_val + "://" + ui.local_ip + ":" + str(ui.local_port) + "/"
                    )
        ...
        if nm:
            if nm.startswith("http"):
                ...
                if get_bytes:
                    self.send_header(
                        "Content-Range",
                        "bytes " + str(get_bytes) + "-" + str(size) + "/" + str(size),
                    )
                self.send_header("Connection", "close")
                self.end_headers()
                # print(size)
    ...
    def process_POST(self):
        ...
        if self.path.startswith("/save_playlist="):
            ...
            self.path.startswith("/sending_playlist")
            ...
            for i in odict:
                val = odict[i]
                lines.append(
                    "{}\t{}\t{}".format(val["title"], val["url"], val["artist"])
                )
            ...
            if isinstance(val, dict):
                ...
                if sample_url:
                    ...
                    fanart_url = (
                        n.scheme + "://" + n.netloc + "/" + "get_current_background"
                    )
            if self.path.startswith("/sending_playlist"):
                ...
                self.nav_signals.total_navigation(
                    "PlayLists", "PlayLists", "TMP_PLAYLIST", False
                )
            else:
            ...
            self.path.startswith("/sending_subtitle") and ui.pc_to_pc_casting == "slave"
        ):
            content = self.rfile.read(int(self.headers["Content-Length"]))
            if isinstance(content, bytes):
                content = str(content, "utf-8")
    ...
    def do_init_function(self, type_request=None):
        ...
        if ui.media_server_key and not ui.media_server_cookie:
            ...
            if client_addr in ui.local_auth_arr:
                ...
                self.final_message(
                    b"Session Established, Now reload page again", set_cookie_id
                )
                cookie_set = True
            ...
            if cookie_set or cookie_verified:
                if ipaddress.ip_address(client_addr).is_private:
                    ...
                    if my_ip:
                        if type_request == "get":
                            self.get_the_content(
                                path, get_bytes, my_ip_addr=my_ip, play_id=playlist_id
                            )
                        elif type_request == "head":
                            self.process_HEAD()
                        elif type_request == "post":
    ...
    def do_GET(self):
        if (
            ...
            self.path.startswith("/master_abs_path=")
            ...
            if ui.pc_to_pc_casting == "master":
                ...
                try:
                    get_bytes = int(
                        self.headers["Range"].split("=")[1].replace("-", "")
                    )
                except Exception as e:
                    get_bytes = 0
                if allow_access:
        ...
        if nm.startswith("http"):
            ...
            if get_bytes or upper_range is not None:
                if upper_range is None:
                    upper_range = size - 1
                logger.info(
                    "...sending range...{0}-{1}/{2}".format(
                        get_bytes, upper_range, size
                    )
                )
                self.send_header(
                    "Content-Range",
    ...
    def triggerBookmark(self, row):
        ...
        arg_dict = ui.get_parameters_value(s="site", o="opt", sn="siteName", n="name")
        site = arg_dict["site"]
        opt = arg_dict["opt"]
    ...
    def create_playlist(
        ...
        site_pls = False
        if site.lower().startswith("playlist") or (
            site.lower().startswith("music")
        ...
        for i in range(len(epnArrList)):
            try:
                ...
                n_art = k.split("	")[-1]
                if (
                    n_art.startswith("http")
                    or n_art.startswith('"http')
                    or n_art.lower() == "none"
                ):
                    n_art = "NONE"
                book_mark = (
                ...
                if site_pls:
                    ...
                    if "##" in k:
                        ...
                        n_art = ""
                n_url_file = getdb.get_file_name_from_bookmark(
                    site, site_option, name, i, epnArrList
                )
                logger.info(name)
                if (
                    site.lower() == "video"
                    ...
                    if site_pls:
                        new_j = k.split("	")[1]
                        if new_j.startswith("abs_path") or new_j.startswith(
                            "relative_path"
                        ):
                            j = new_j
                ...
                if play_id:
                    out = (
                        ...
                        + str(my_ipaddress)
                        + ":"
                        + str(ui.local_port_stream)
                        + "/"
                        + j
                        + "&pl_id="
                        + play_id
                        + "/"
                        + urllib.parse.quote(n_url_name.replace("/", "-"))
                    )
                else:
                    out = (
                        http_val
                ...
                if self.path.endswith(".pls"):
                    pls_txt = pls_txt + "\nFile{0}={1}\nTitle{0}={2}-{3}\n".format(
                        str(i), out, n_art, n_out
                    )
                elif self.path.endswith(".htm") or self.path.endswith(".html"):
                    if site.lower() == "video":
                        pls_txt = (
                            pls_txt
                            + '<li data-mp3="{2}" data-num="{3}"><img src="{4}" width="128">{1}</li>'.format(
                                n_art, n_out, out, str(i + 1), out + ".image"
                            )
                        )
                    else:
                        pls_txt = (
                            pls_txt
                            + '<li data-mp3="{2}" data-num="{3}"><img src="{4}" width="128">{0} - {1}</li>'.format(
                                n_art, n_out, out, str(i + 1), out + ".image"
                            )
                        )
                else:
        ...
        if self.path.endswith(".pls"):
            ...
            if os.path.exists(playlist_htm):
                ...
                for i in html_default_arr:
                    new_field = new_field + '<option value="{0}">{1}</option>'.format(
                        i.lower(), i
                    )
                new_field = '<select id="site" onchange="siteChange()">{0}</select>'.format(
                    new_field
                )
                logger.info(new_field)
                pls_txt = pls_txt.replace(
                    '<select id="site" onchange="siteChange()"></select>', new_field
                )
                extra_fields = self.get_extra_fields()
                logger.info(extra_fields)
                pls_txt = re.sub(
                    '<div id="site_option" hidden></div>', extra_fields, pls_txt
                )
    ...
    def get_extra_fields(self):
        ...
        extra_fields = "RemoteField:{0};RemoteControl:{1};Thumbnails:{2};WebControl:{3};".format(
            ui.remote_control_field,
            ui.remote_control,
            ui.show_client_thumbnails,
            ui.web_control,
        )
        for i in html_default_arr:
            if i.lower() == "video":
                ...
                extra_fields = (
                    extra_fields + "Video:Available;Video:History;" + category_list
                )
            elif i.lower() == "music":
                extra_fields = (
                    extra_fields
                    + "Music:Artist;Music:Album;Music:Directory;Music:Playlist;"
                )
    ...
    def get_the_content(self, path, get_bytes, my_ip_addr=None, play_id=None):
        ...
        if (
            path.lower().startswith("stream_continue")
            ...
            for i in range(len(new_arr)):
                try:
                    k = new_arr[i]
                    n_url_file = ui.if_file_path_exists_then_play(
                        k, ui.list2, play_now=False
                    )
                    if (
                        site.lower() == "video"
                        or site.lower() == "music"
                    ...
                    if path.endswith(".pls"):
                        ...
                        if site.lower() == "video":
                            pls_txt = (
                                pls_txt
                                + '<li data-mp3="{2}" data-num="{3}" draggable="true" ondragstart="drag_start(event)" ondragend="drag_end(event)" ondragover="drag_over(event)" ondragleave="drag_leave(event)" ondragenter="drag_enter(event)" ondrop="on_drop(event)" title="{1}">{1}</li>'.format(
                                    n_art, n_out, out, str(i + 1)
                                )
                            )
                        else:
                            pls_txt = (
                                pls_txt
                                + '<li data-mp3="{2}" data-num="{3}" draggable="true" ondragstart="drag_start(event)" ondragend="drag_end(event)" ondragover="drag_over(event)" ondragleave="drag_leave(event)" ondragenter="drag_enter(event)" ondrop="on_drop(event)" title="{0} - {1}">{0} - {1}</li>'.format(
                                    n_art, n_out, out, str(i + 1)
                                )
                            )
                    else:
            ...
            if path.endswith(".pls"):
                ...
                if os.path.exists(playlist_htm):
                    ...
                    for i in html_default_arr:
                        new_field = (
                            new_field
                            + '<option value="{0}">{1}</option>'.format(i.lower(), i)
                        )
                    ...
                    new_field_select = '<select id="first_select" onchange="siteChangeTop()">{0}</select>'.format(
                        copy_new_field
                    )
            ...
            if path.endswith(".htm") or path.endswith(".html"):
                ...
                self.send_header(
                    ...
                    "attachment; filename={}".format(
                        playlist_file_name.encode("utf-8")
                    ),
                )
            size = len(pls_txt)
            # size = size - get_bytes
            ...
            if ui.https_media_server:
                http_val = "https"
            n_url = (
                http_val + "://" + str(my_ipaddress) + ":" + str(ui.local_port_stream)
            )
            n_url_name = "now_playing"
            n_url_new = base64.b64encode(bytes(n_url, "utf-8"))
            n_url = str(n_url_new, "utf-8")
            ...
            if path.endswith(".pls"):
                ...
                pls_txt = pls_txt + '<li data-mp3="{2}">{0} - {1}</li>'.format(
                    n_art, n_out, out
                )
            else:
                pls_txt = pls_txt + "#EXTINF:0, {0} - {1}\n{2}\n".format(
            ...
            path.lower().startswith("site=")
            or path.startswith("get_previous_playlist")
            or path.startswith("get_next_playlist")
            or path.startswith("get_last_playlist")
            ...
            if get_pls:
                if ret_val is not None:
                    path = ret_val
                    logger.debug(
                        "total={0}::ptr={1}".format(
                            ui.navigate_playlist_history.get_total,
                            ui.navigate_playlist_history.get_ptr,
                        )
                    )
            ...
            for i in new_arr:
                ...
                if i.startswith("site="):
                    ...
                    if (
                        srch.endswith(".pls")
                        or srch.endswith(".m3u")
                        or srch.endswith(".htm")
                        or srch.endswith(".html")
                    ):
            ...
            if (
                st.startswith("video")
                or st.startswith("music")
                or st.startswith("playlist")
            ):
            ...
            if st and st_o and srch and not pls_cache:
                ...
                if original_path_name:
                    pls_txt = self.create_option_playlist(st, st_o, original_path_name)
            self.send_response(200)
            if path.endswith(".htm") or path.endswith(".html"):
                self.send_header("Content-type", "text/html; charset=utf-8")
            else:
                self.send_header("Content-type", "audio/mpegurl")
                self.send_header(
                    "Content-Disposition", "attachment; filename={}".format(srch)
                )
            if not pls_cache:
                pls_txt = bytes(pls_txt, "utf-8")
            size = len(pls_txt)
            ...
            if ui.remote_control and ui.remote_control_field:
                if self.playlist_shuffle_list and shuffle_list:
                    ui.epn_arr_list = self.playlist_shuffle_list.copy()
                self.nav_signals.total_navigation(
                    st_arr[0], st_arr[1], st_arr[2], shuffle_list
                )
            ...
            path.startswith("playlist_")
            or path.startswith("queueitem_")
            or path.startswith("queue_remove_item_")
        ):
            try:
                ...
                if ui.remote_control and ui.remote_control_field:
                    if ui.web_control == "master":
                        ...
                        msg = (
                            ...
                            + "::"
                            + str(ui.list2.currentRow() + 1)
                            + "::"
                            + str(len(ui.queue_url_list))
                            + "::"
                            + str(ui.epn_name_in_list)
                        )
                    else:
            ...
            try:
                ...
                if nm.startswith("http") or nm.startswith("ytdl:"):
                    ...
                    if nm.startswith(n_url):
                        ...
                        ("youtube.com" in nm or nm.startswith("ytdl:"))
                        and not self.path.endswith(".subtitle")
                        and not self.path.endswith(".getsub")
                        and not self.path.endswith(".image")
                    ):
                        nm = ui.yt.get_yt_url(
                            nm,
                        ...
                        if "::" in nm:
                            ...
                            vid = nm_arr[0]
                            aud = nm_arr[1]
                            if ui.client_yt_mode == "music" and not aud.endswith(
                                ".vtt"
                            ):
                                nm = aud
        ...
        elif path.startswith("relative_path=") or path.startswith(
                    ...
                    if ui.remote_control and ui.remote_control_field:
                        if "playlist_index=" in self.path:
                            ...
                            if row_num_val.isnumeric():
                                ...
                                mode = row_num_val
                            logger.info(
                                "{0}--row--num--playlist--{1}".format(row_num, mode)
                            )
                        ...
                        if "&pl_id=" in self.path:
                            ...
                            logger.debug("\nplaylist--id--{0}\n".format(nm))
                        if self.path.endswith(".subtitle"):
                            loc = get_torrent_download_location(
                                old_nm, home, ui.torrent_download_folder
                            )
                            new_path = self.path.rsplit(".", 1)[0]
                            if new_path.endswith(".reload"):
            ...
            try:
                if ui.torrent_handle:
                    ...
                    msg = (
                        "no torrent handle, first start torrent session before removing"
                    )
                msg = bytes(msg, "utf-8")
            except Exception as e:
                print(e)
            ...
            try:
                ...
                msg = "Download Speed Limit:{0}, Upload Speed Limit:{1} SET".format(
                    down_speed, up_speed
                )
                msg = bytes(msg, "utf-8")
                self.final_message(msg)
            ...
            if new_url.startswith("http") or new_url.startswith("magnet"):
                ...
                if var_name:
                    ...
                    msg = "Wrong Parameters: Don't do that again without selecting Torrent from the list".format(
                        var_name
                    )
                    msg = bytes(msg, "utf-8")
                    self.final_message(msg)
            ...
            txt = "artist={3}:\npls-name={0}:\ntitle={1}:\nlink={2}".format(
                pls_name, title, data_link, artist
            )
            logger.info(txt)
            ...
            pls_name = arr[0]
            pls_num = arr[1]
            msg = "Deleting Number {0} Entry from playlist: {1}".format(
                pls_num, pls_name
            )
            ...
            try:
                ...
                if url and pls:
                    ...
                    if url.startswith("http"):
                        ...
                        logger.info(
                            "---1914---val={0}--url={1}--pls={2}".format(val, url, pls)
                        )
                        if val:
    ...
    def process_image_url(self, path):
        ...
        if (
            ...
            and not path.startswith("relative_path=")
        ) or got_http_image:
        ...
        if status == "original":
            if check_local_sub:
                if os.name == "nt":
                    try:
                        content = open(
                            check_local_sub, mode="r", encoding="utf-8-sig"
                        ).read()
                    except Exception as err:
                        logger.error(err)
            ...
            if check_local_sub is None:
                check_local_sub = self.check_local_subtitle(
                    sub_name_path + ".mkv", external=True
                )
        ...
        if url and pls:
            ...
            if url.startswith("http") and "youtube.com" in url:
                ...
                if not yt_playlist and pls_path:
                    ...
                    ut = soup.findAll(
                        "li", {"class": "yt-uix-scroller-scroll-unit vve-check"}
                    )
                    if ut_c:
                        ut = ut_c + ut
                    arr = []
                ...
                if mode == "offline":
                    ...
                    if "::" in nm:
                        ...
                        if ui.client_yt_mode == "music" and not aud.endswith(".vtt"):
                            ...
                            nm = vid
                    self.process_offline_mode(
                        nm, pls, title, msg=False, captions=sub_title, url=url
                    )
            else:
                try:
                    req = urllib.request.Request(
    ...
    def run(self):
        ...
        try:
            ...
            if not ui.https_media_server:
                ...
                self.httpd = ThreadedHTTPServerLocal(
                    server_address, HTTPServer_RequestHandler
                )
                ...
                self.httpd.socket = ssl.wrap_socket(
                    self.httpd.socket, certfile=cert, ssl_version=ssl.PROTOCOL_TLSv1_2
                )
                # self.set_local_ip_val()
...
def media_server_started(val):
    global ui
    if val == "http":
        msg = "Media Server Started.\nWeb interface is available at\nhttp://{0}:{1}/stream_continue.htm".format(
            ui.local_ip_stream, str(ui.local_port_stream)
        )
        send_notification(msg)
    elif val == "https":
        msg = "Media Server Started with SSL support.\nWeb interface available at \nhttps://{0}:{1}/stream_continue.htm".format(
            ui.local_ip_stream, str(ui.local_port_stream)
        )
        send_notification(msg)

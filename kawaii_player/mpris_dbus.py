...
class MprisServer(dbus.service.Object):
    def __init__(self, ui, home, tray, new_tray):
        ...
        bus = dbus.service.BusName(AW_MPRIS_BUS_NAME, bus=dbus.SessionBus())
        super().__init__(bus, MPRIS_OBJECT_PATH)

        self._properties = dbus.Dictionary(
            {"DesktopEntry": "kawaii-player", "Identity": "kawaii-player",},
            signature="sv",
        )
    ...
    @dbus.service.method(
        MPRIS_MEDIAPLAYER_PLAYER_INTERFACE, in_signature="", out_signature=""
    )
    def Play(self):
        print("Play Song")
    ...
    def _emitMeta(self, info, site, epnArrList):
        ...
        if epnArrList and site in ["Music", "PlayLists"]:
            try:
                ...
                if "internet-radio#" in info:
                    ...
                    t = t.replace("#", "")
                    print(art, t)
                if (site == "Music" and self.ui.list3.currentItem()) or (
                    site == "PlayLists"
                ):
                    if (
                        site == "Music"
                        and self.ui.list3.currentItem().text().lower() == "playlist"
                    ) or (site == "PlayLists"):
                        artist = art
                        ...
                        if not queue_list:
                            pls_entry = (
                                self.ui.list2.currentItem().text().replace("#", "")
                                + ".jpg"
                            )
                        else:
                            pls_entry = t.replace("#", "")
                        ...
                        img_place = os.path.join(
                            self.home, "thumbnails", "PlayLists", pls, pls_entry
                        )
                        ...
                        if artist.startswith(self.ui.check_symbol):
                            artist = artist[1:]
                        art_u = os.path.join(
                            self.home, "Music", "Artist", artist, "poster.jpg"
                        )
        ...
        self.PropertiesChanged(MPRIS_MEDIAPLAYER_PLAYER_INTERFACE, props, [])
    ...
    def GetAll(self, interface):
        if interface == MPRIS_MEDIAPLAYER_PLAYER_INTERFACE:
            ...
            raise dbus.exceptions.DBusException(
                ...
                "The Foo object does not implement the %s interface" % interface,
            )

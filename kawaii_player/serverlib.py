...
class ServerLib:
    ...
    def get_file_name_from_bookmark(self, site, site_option, name, row, epnArrList):
        ...
        if (
            ...
            file_name_mkv = os.path.join(
                ui.default_download_location, title, new_epn_mkv
            )
            file_name_mp4 = os.path.join(
                ui.default_download_location, title, new_epn_mp4
            )
        elif site.lower() == "playlists" or (
            site.lower() == "music" and site_option.lower().startswith("playlist")
        ):
            title = name
    ...
    def options_from_bookmark(self, site, site_option, search_term, search_exact=None):
        ...
        if (
            not site.lower().startswith("playlist")
            and site.lower() != "music"
        ...
        if (
            site.lower() != "local"
            and site.lower() != "music"
            and site.lower() != "subbedanime"
            and site.lower() != "dubbedanime"
            and not site.lower().startswith("playlist")
            and site.lower() != "video"
            and site.lower() != "none"
        ):
            ...
            if send_list_direct:
                ...
                original_path_name[:] = []
                if (
                    music_opt.lower() == "artist"
                    or music_opt.lower() == "album"
                    or music_opt.lower() == "title"
                    or music_opt.lower() == "fav-artist"
                    or music_opt.lower() == "fav-album"
                ):
                    for i in artist:
                        original_path_name.append(i)
                elif (
                    music_opt.lower() == "directory"
                    or music_opt.lower() == "fav-directory"
                ):
                    for i in artist:
                        original_path_name.append(i)
                    ...
                    for i in artist:
                        original_path_name.append(
                            i.replace(".txt", "")
                            + "	"
                            + os.path.join(home, "Playlists", i)
                        )
                    ...
                    for i in line_a:
                        ...
                        try:
                            ...
                            try:
                                ...
                                new_dir = new_name
                            logger.info(
                                "{0}-{1}-{2}".format(search_term, new_name, new_dir)
                            )
                            if search_term and search_term in new_name.lower():
                                original_path_name.append(new_name + "	" + new_dir)
                                m1 = ui.media_data.get_video_db(
                                    video_db, "Directory", new_dir
                                )
                                for i in m1:
            ...
            logger.info(artist)
            if (
                video_opt.lower() != "update" and video_opt.lower() != "updateall"
            ) and not send_list_direct:
                for i in artist:
                    ti = i.split("	")[0]
                    di = i.split("	")[1]
                    if os.path.exists(di):
                        if ti.lower().startswith("season") or ti.lower().startswith(
                            "special"
                        ):
                ...
                original_path_name = sorted(
                    original_path_name, key=lambda x: x.split("	")[0].lower()
                )
    ...
    def listfound_from_bookmark(
        self, site, site_option, search_term, original_path_name, search_exact=None
    ):
        site_var = None
        bookmark = False
        status = site_option
            ...
            if search_term:
                ...
                for i, value in enumerate(original_path_name):
                    ...
                    if (search_term in search_field and not search_exact) or (
                        ...
                        extra_info = ""
                        logger.info(
                            "cur_row={0}, new_name={1}".format(i, new_name_with_info)
                        )
                        ...
                        if (
                            ...
                            hist_site = os.path.join(
                                home, "History", site, siteName, name
                            )
                        else:
            ...
            for index, value in enumerate(original_path_name):
                ...
                if (search_term in search_field and not search_exact) or (
                    ...
                    if music_opt.lower() == "fav-directory":
                        art_n = value
                    if (
                        music_opt.lower() == "playlist"
                        or music_opt.lower() == "playlists"
                    ):
                        pls = value.split("	")[0]
                ...
                if (search_term in search_field and not search_exact) or (
                    ...
                    if not bookmark:
                        ...
                        if new_dir_path is not None:
                            ...
                            m = ui.media_data.get_video_db(video_db, "Bookmark", art_n)
    ...
    def epn_return_from_bookmark(self, tmp_bookmark, from_client=None):
        ...
        if len(tmp_arr) == 7:
            ...
            if local_stream_index >= 0:
                na_me = "&".join(
                    new_tmp_arr[: -(len(new_tmp_arr) - local_stream_index)]
                )
            if row_index >= 0:
                ...
                if vi_deo_local_stream:
                    ...
                    if ui.torrent_serve_thread.isRunning():
                        if ui.torrent_status_thread.isRunning():
                            ...
                            (
                                finalUrl,
                                ui.torrent_status_thread,
                                ui.stream_session,
                                ui.torrent_handle,
                            ) = ui.start_torrent_stream(
                                na_me,
                                row,
                                ui.local_ip + ":" + str(ui.local_port),
                                get_next,
                                ui.torrent_download_folder,
                                ui.stream_session,
                                site_name=si_te,
                    ...
                    finalUrlT = si_te_var.getFinalUrl(
                        na_me, ep_n, mirrorNo, ui.client_quality_val
                    )

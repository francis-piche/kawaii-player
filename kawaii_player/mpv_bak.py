...
class MpvRenderParam(Structure):
    _fields_ = [("type", MpvRenderParamType), ("data", c_void_p)]
...
class MpvOpenglInitParams(Structure):
    _fields_ = [
        ("get_proc_address", CFUNCTYPE(c_void_p, c_void_p, c_char_p)),
...
def generate_params(handle, gl, addr):
    ...
    param_1 = MpvRenderParam(
        MpvRenderParamType.MPV_RENDER_PARAM_API_TYPE, cast(api_string, c_void_p)
    )
    gl_space = MpvOpenglInitParams(OpenGlCbGetProcAddrFn(gl), None, None)
    gl_params = byref(gl_space)
    param_2 = MpvRenderParam(
        MpvRenderParamType.MPV_RENDER_PARAM_OPENGL_INIT_PARAMS,
        cast(gl_params, c_void_p),
    )
...
try:
    ...
    _handle_func(
        ...
        [POINTER(POINTER(MpvRenderContext)), MpvHandle, POINTER(MpvRenderParam)],
        c_int,
        None,
        None,
    )
    _handle_func(
        ...
        [POINTER(MpvRenderContext), POINTER(MpvRenderParam)],
        c_int,
        None,
        None,
    )
    _handle_func(
        "mpv_render_context_free", [POINTER(MpvRenderContext)], c_int, None, None
    )
    _handle_func(
        "mpv_render_context_report_swap", [POINTER(MpvRenderContext)], None, None, None
    )
...
class MPV(object):
    ...
    def wait_for_property(
        self, name, cond=lambda val: val, level_sensitive=True, timeout=None
    ):

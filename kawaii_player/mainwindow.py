...
class MainWindowWidget(QtWidgets.QWidget):
    ...
    def dropEvent(self, event):
        ...
        for url in urls:
            ...
            if "." in url:
                ...
                ext = ext.lower()
            rect = QtCore.QRect(
                ui.label.x(), ui.label.y(), ui.label.width(), ui.label.height()
            )
            pos = event.pos()
            ...
            if ext and ext in ["jpg", "jpeg", "png"] and not url.startswith("http"):
                ...
                url = url.replace(" ", "%20")
                ui.vnt.head(
                    url, onfinished=partial(self.process_dropped_url, poster_drop, name)
                )
            else:
    ...
    def process_dropped_url(self, *args):
        ...
        if result.error is None:
            if (
                result.content_type in ["image/jpeg", "image/png", "image/webp"]
                or url.endswith(".jpg")
                or url.endswith(".png")
            ):
                ui.vnt.get(
                    url,
                    out=out_file,
                    onfinished=partial(self.download_fanart, poster_drop, nm),
                )
            else:
                ui.gui_signals.poster_drop(poster_drop, url, nm)
        elif url.endswith(".jpg") or url.endswith(".png"):
            ui.vnt.get(
                url, out=out_file, onfinished=partial(self.download_fanart, poster_drop)
            )

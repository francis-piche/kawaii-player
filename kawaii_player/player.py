...
class PlayerWidget(QtWidgets.QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        if (
            not self.ui.float_window.isHidden()
            and self.ui.new_tray_widget.remove_toolbar
        ):
            if self.ui.float_timer.isActive():
                self.ui.float_timer.stop()
            if (
                self.ui.new_tray_widget.cover_mode.text()
                == self.ui.player_buttons["up"]
            ):
        ...
        if self.player_val in self.ui.playback_engine:
            ...
            if api == "libmpv" and value:
                ...
                if self.ui.fullscreen_video:
                    ...
                    status_string = "{} / {} Title: {}".format(
                        fn(self.ui.progress_counter),
                    ...
                    if self.ui.player_val == "libmpv":
                        ...
                        msg = bytes(
                            "show-text '{}' 1000".format(status_string), "utf-8"
                        )
    ...
    def keyPressEvent(self, event):
        ...
        if self.player_val.lower() in ["mpv", "libmpv"]:
            ...
            if (
                ...
                event.modifiers() == QtCore.Qt.ShiftModifier or self.event_dict["shift"]
            ):
                self.event_dict["ctrl"] = False
                self.event_dict["alt"] = False
    ...
    def change_aspect_ratio(self, key=None):
        ...
        logger.info(
            "aspect:{0}::value:{1}".format(self.ui.mpvplayer_aspect_cycle, aspect_val)
        )
        if aspect_val == "-1":
        ...
        if self.player_val in ["mpv", "mplayer"]:
            ...
            if self.ui.final_playing_url in self.ui.history_dict_obj:
                (
                    ...
                    vol,
                    asp,
                ) = self.ui.history_dict_obj.get(self.ui.final_playing_url)
                asp = aspect_val
    ...
    def keyPressEventOld(self, event):
        if (
            ...
            self.ui.mpvplayer_aspect_cycle = (self.ui.mpvplayer_aspect_cycle + 1) % 5
            aspect_val = self.ui.mpvplayer_aspect.get(
                str(self.ui.mpvplayer_aspect_cycle)
            )
            logger.info(
                "aspect:{0}::value:{1}".format(
                    self.ui.mpvplayer_aspect_cycle, aspect_val
                )
            )
            if aspect_val == "-1":
                show_text_val = "Original Aspect"
            ...
            if self.player_val == "mpv":
                ...
                if self.ui.total_file_size:
                    ...
                    if self.ui.final_playing_url.startswith(
                        "http"
...
class KeyBoardShortcuts:
    ...
    def get_default_keys(self):
        ...
        default_key_map["right"] = "osd-msg-bar seek +10"
        default_key_map["left"] = "osd-msg-bar seek -10"
        default_key_map["up"] = "osd-msg-bar seek +60"
        default_key_map["down"] = "osd-msg-bar seek -60"

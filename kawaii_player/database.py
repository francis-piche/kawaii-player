...
class MediaDatabase:
    ...
    def get_video_db(self, music_db, queryType, queryVal):
        ...
        if q.lower() == "directory":
            if not qVal:
                cur.execute(
                    "SELECT distinct Title, Directory FROM Video order by Title"
                )
            else:
            ...
            if not cat_type:
                ...
                self.logger.info(
                    "Category {} not Available Hence sending Others --123---".format(q)
                )
            if not qVal:
                try:
                    cur.execute(
                        "SELECT distinct Title, Directory FROM Video Where Category=? order by Title",
                        (cat_type,),
                    )
                except Exception as err:
                        ...
                        for ii in m:
                            if "." in ii:
                                ...
                                if ext in self.ui.video_type_arr:
                                    if q.lower() == "history":
                                        access_time = self.ui.history_dict_obj.get(
                                            file_path
                                        )
                                        if access_time:
    ...
    def create_update_video_db(
        self, video_db, video_file, video_file_bak, update_progress_show=None
    ):
        ...
        if os.path.exists(video_db):
            ...
            cur = conn.cursor()
            cur.execute(
                """CREATE TABLE Video(Title text, Directory text, FileName text, EP_NAME text, Path text primary key, EPN integer, Category integer)"""
            )
            for i in lines:
                i = i.strip()
                if i:
    ...
    def alter_table_and_update(self, version):
        if version <= (2, 0, 0, 0) and version > (0, 0, 0, 0):
            ...
            try:
                ...
                for i in rows:
                    self.logger.info(
                        "Databse Schema Changed updating Entries::{0}".format(i)
                    )
                    if i:
                        path = i[0].lower()
    ...
    def update_video_count(self, qType, qVal, rownum=None):
        ...
        cur = conn.cursor()
        self.logger.info(
            "{0}:{1}:{2}::::::database.py:::240".format(qType, qVal, rownum)
        )
        if qType == "mark":
            # qVal = '"'+qVal+'"'
    ...
    def update_on_start_video_db(
        self, video_db, video_file, video_file_bak, video_opt, update_progress_show=None
    ):
        if (update_progress_show is None or update_progress_show) and self.ui:
            self.ui.text.setText("Wait..Updating Video Database")
            QtWidgets.QApplication.processEvents()
        ...
        l1 = len(m_files)
        l2 = len(m_files_old)
        m = list(set(m_files) - set(m_files_old)) + list(
            set(m_files_old) - set(m_files)
        )
        m.sort()
    ...
    def get_music_db(self, music_db, queryType, queryVal):
        ...
        if q == "Artist":
            ...
            if not qVal:
                cur.execute(
                    "SELECT Distinct Directory FROM Music Where Favourite='yes'"
                )
            else:
                # cur.execute('SELECT Artist, Title, Path FROM Music Where Directory="'+qVal+'"')
                qr = "SELECT Artist, Title, Path FROM Music Where Directory=?"
            ...
            print(q)
            cur.execute(
                "SELECT Artist, Title, Path FROM Music order by Playcount desc limit 50"
            )
        elif q == "Last 50 Newly Added":
            print(q)
            cur.execute(
                "SELECT Artist, Title, Path FROM Music order by Modified desc limit 50"
            )
        elif q == "Last 50 Played":
            print(q)
            cur.execute(
                "SELECT Artist, Title, Path FROM Music order by LastPlayed desc limit 50"
            )
        elif q == "Search":
            print(q)
    ...
    def update_music_count(self, qType, qVal):
        ...
        if qType == "count":
            ...
            try:
                ...
                print(err)
                qr = (
                    'Update Music Set LastPlayed=?, Playcount=? Where Path="'
                    + qVal
                    + '"'
                )
                q1 = datetime.datetime.now()
    ...
    def create_update_music_db(
        self, music_db, music_file, music_file_bak, update_progress_show=None
    ):
        if (update_progress_show is None or update_progress_show) and self.ui:
        ...
        if os.path.exists(music_db):
            ...
            cur = conn.cursor()
            cur.execute(
                """CREATE TABLE Music(Title text, Artist text, Album text, Directory text, Path text primary key, Playlist text, Favourite text, FavouriteOpt text, Playcount integer, Modified timestamp, LastPlayed timestamp)"""
            )
            for k in lines:
                j = k.split("	")[0]
                i = j.strip()
                ...
                try:
                    ...
                    t = t + 1
                    if (
                        update_progress_show is None or update_progress_show
                    ) and self.ui:
    ...
    def get_tag_lib(self, path):
        ...
        if ar1 == "Unknown" or al1 == "Unknown":
            self.logger.info(
                "Error Artist={0}:Album={1}:Path={2}".format(ar1, al1, path)
            )
                                ...
                                if file_ext.lower() in self.ui.music_type_arr:
                                    ...
                                    if os.path.isfile(path):
                                        s = (
                                            path
                                            + "	"
                                            + (str(os.path.getmtime(path))).split(".")[
                                                0
                                            ]
                                        )
                                        m_files.append(s)

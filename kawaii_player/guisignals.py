...
class GUISignals(QtCore.QObject):
    ...
    def generate_preview(
        self, picn, length, change_aspect, tsec, counter, x, y, source_val
    ):
        self.preview_signal.emit(
            picn, length, change_aspect, tsec, counter, x, y, source_val
        )
    ...
    def check_master_mode(value):
        def real_deco(func):
            def wrapper(*args, **kargs):
                gui = args[0]
                if (
                    gui.extra_toolbar_control == "slave"
            ...
            def wrapper(*args, **kargs):
                ...
                if (
                    ...
                    if ui.list2.currentItem():
                        ui.list2.start_pc_to_pc_casting(
                            "playlist", ui.list2.currentRow()
                        )
                    else:
                        func(*args, **kargs)
                else:
    ...
    def preview_generated_new(
        self, picn, length, change_aspect, tsec, counter, x, y, source_val
    ):
        ui.slider.preview_generated(
            picn, length, change_aspect, tsec, counter, x, y, source_val
        )
    ...
    def track_slave_status(self, val):
        ...
        if len(val_arr) > 3 and ui.mpvplayer_val.processId() == 0:
            ...
            if (
                counter in range(0, 5) and row != ui.list2.currentRow()
            ) or self.first_time:
    ...
    def apply_fanart_widget(self, fanart, theme):
        QtCore.QTimer.singleShot(
            100, partial(ui.set_mainwindow_palette, fanart, theme=theme)
        )
    ...
    def control_slave_playback(self, param, val):
        ...
        if param_type == "gsbc":
            ...
            if slider_name == "subscale":
                ...
                slider = eval(
                    "ui.frame_extra_toolbar.subtitle_{}_slider".format(slider_name)
                )
                slider.setValue(int(slider_val))
            ...
            if widget_name == "widget" and widget_val in widget_list:
                ...
                ui.frame_extra_toolbar.apply_slave_subtitile_effects(
                    widget_name, widget_val
                )
            elif widget_name in [
                "checkbox_bold",
                "checkbox_italic",
    ...
    def show_login_box(self, url, pls, verify):
        LoginPCToPC(
            MainWindow, url, ui, pls, verify, onfinished=ui.list2.post_pc_processing
        )

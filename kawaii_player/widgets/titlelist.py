...
class TitleListWidget(QtWidgets.QListWidget):
    ...
    def mouseMoveEvent(self, event):
        if (
            ui.auto_hide_dock
            and not ui.dockWidget_3.isHidden()
        ...
        if (
            ...
            if bookmark:
                ...
                if site == "SubbedAnime" or site == "DubbedAnime":
                    ...
                    if os.path.exists(
                        os.path.join(home, "History", site, "history.txt")
                    ):
                        file_path = os.path.join(home, "History", site, "history.txt")
                row = self.currentRow()
                if row == 0:
                    ...
                    prev_row = row - 1
                ui.original_path_name[row], ui.original_path_name[prev_row] = (
                    ui.original_path_name[prev_row],
                    ui.original_path_name[row],
                )
                if os.path.exists(file_path):
                    write_files(file_path, ui.original_path_name, line_by_line=True)
                self.clear()
            ...
            if bookmark:
                ...
                if row == (self.count() - 1):
                    ...
                    next_row = row + 1
                ui.original_path_name[row], ui.original_path_name[next_row] = (
                    ui.original_path_name[next_row],
                    ui.original_path_name[row],
                )
                if os.path.exists(file_path):
                    write_files(file_path, ui.original_path_name, line_by_line=True)
    ...
    def get_all_information(self):
        ...
        backend_dict = {
            "duckduckgo+tvdb": "tvdb+ddg",
            "duckduckgo+tmdb": "tmdb+ddg",
            "google+tvdb": "tvdb+g",
            "google+tmdb": "tmdb+g",
            "tvdb": "tvdb",
            "tmdb": "tmdb",
            ...
            try:
                ...
                if video_dir and self.currentRow() > 0:
                    if self.currentRow() < len(ui.original_path_name):
                        video_dir = ui.original_path_name[self.currentRow()].split(
                            "\t"
                        )[-1]
    ...
    def update_video_category(self, site, txt):
        ...
        if site.lower() == "video" and self.selectedItems() and not bookmark:
            ...
            if category:
                ...
                conn.commit()
                conn.close()
                msg = "{0} Successfully Added to category: {1}".format(
                    notify_title, txt
                )
            else:
    ...
    def delete_category_video(self, site, category):
        ...
        if cat_val and site.lower() == "video":
            ...
            logger.info(
                "Category Dict: {0}\nCategory Array: {1}".format(
                    ui.category_dict, ui.category_array
                )
            )
            msg = "Category {0} Removed; Removed Items are in Others Category".format(
                category
            )
            send_notification(msg, code=0)
        else:
            ...
            if site == "Video":
                ...
                if not os.path.isfile(new_pls_name):
                    ...
                    if itemlist:
                        ...
                        if os.path.isfile(old_pls_name) and not os.path.isfile(
                            new_pls_name
                        ):
                ...
                if itemlist:
                    ...
                    if site.lower() == "subbedanime" or site.lower() == "dubbedanime":
                        history_txt = os.path.join(
                            home, "History", site, siteName, "history.txt"
                        )
                        file_dir_new = os.path.join(
                            home, "History", site, siteName, item
                        )
                        file_dir_old = os.path.join(
                            home, "History", site, siteName, old_name
                        )
                    else:
                    ...
                    old_history = set(
                        [i.strip().split("\t")[0] for i in old_history if i.strip()]
                    )
                    if item not in old_history:
                        ...
                        ui.original_path_name[row] = tmp
                        write_files(
                            history_txt, ui.original_path_name, line_by_line=True
                        )
                        itemlist.setText(item)
                        if (
                            os.path.exists(file_dir_old)
                            and file_dir_old != file_dir_new
                        ):
                            if not os.path.exists(file_dir_new):
                                shutil.move(file_dir_old, file_dir_new)
                                logger.info(
                                    "\nmoving {0}::to::{1}\n".format(
                                        file_dir_old, file_dir_new
                                    )
                                )
                                if ui.video_local_stream:
                                    torrent_file_old = file_dir_old + ".torrent"
                                    torrent_file_new = os.path.join(
                                        file_dir_new, "title.torrent"
                                    )
                                    if os.path.isfile(
                                        torrent_file_old
                                    ) and not os.path.isfile(torrent_file_new):
                                        shutil.move(torrent_file_old, torrent_file_new)
                                    elif os.path.isfile(
                                        torrent_file_old
                                    ) and os.path.isfile(torrent_file_new):
                                        os.remove(torrent_file_old)
                                if os.path.exists(
                                    thumbnail_dir_old
                                ) and not os.path.exists(thumbnail_dir_new):
                                    shutil.move(thumbnail_dir_old, thumbnail_dir_new)
                            else:
                                shutil.rmtree(file_dir_old)
                                msg = "\nFolder: {0} already exists thus removing {1}\n".format(
                                    file_dir_new, file_dir_old
                                )
                                logger.info(msg)
                    else:
                        msg = "Choose another name. Title: {} already exists".format(
                            old_name
                        )
                        send_notification(msg)
        ...
        if site == "Music":
            ...
            if action == fav:
                ...
                if "/" in name:
                    ...
                    nam = name
                ui.reviewsWeb(
                    srch_txt=nam, review_site="last.fm", action="search_by_name"
                )
                ...
                if ui.list3.currentItem():
                    ...
                    if nam:
                        ...
                        fanart = os.path.join(
                            home, "Music", "Artist", nam, "fanart.jpg"
                        )
                        ...
                        if os.path.exists(dir_n):
                            if action == delInfo:
                                ...
                                m = os.listdir(dir_n)
                                for i in m:
                                    if i.startswith("fanart") or i.startswith(
                                        "original-fanart"
                                    ):
                                        os.remove(os.path.join(dir_n, i))
                                m = os.listdir(TMPDIR)
                                ...
                                ui.videoImage(
                                    picn,
                                    os.path.join(
                                        home, "Music", "Artist", nam, "thumbnail.jpg"
                                    ),
                                    fanart,
                                    "",
                                )
            ...
            pls = sorted(
                pls,
                key=lambda x: os.path.getmtime(os.path.join(book_dir, x)),
                reverse=True,
            )
            item_m = []
            for i in pls:
            ...
            if action in item_m:
                ...
                cat_list = [
                    ui.category_array[i] for i in range(5, len(ui.category_array))
                ]
                if cat_list:
                    item, ok = QtWidgets.QInputDialog.getItem(
                        MainWindow,
                        "Input Dialog",
                        "Select Extra Category To Remove from Video Section",
                        cat_list,
                        0,
                        False,
                    )
                ...
                param_dict = ui.get_parameters_value(b="bookmark", o="opt", s="site")
                opt = param_dict["opt"]
                bookmark = param_dict["bookmark"]
                site = param_dict["site"]
                if (
                    (site == "Local" or site == "PlayLists")
                    or bookmark
                    or opt == "History"
                    or site == "Video"
                ):
                ...
                for i in ui.history_dict_obj:
                    if (i.startswith("#") or os.path.isfile(i)) and i not in [
                        "#LAST@TITLE",
                        "#LIBMPV@WATCH#LATER",
                    ]:

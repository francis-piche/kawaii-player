...
class FloatWindowWidget(QtWidgets.QWidget):
    ...
    def __init__(self, ui_widget=None, tray_widget=None, logr=None):
        ...
        self.h_mode.setToolTip(
            "<html>Turn on/off auto hide behaviour of toolbar. Use ESC key, to show/hide toolbar</html>"
        )
        ...
        self.f.setStyleSheet(
            """QFrame{font: bold 15px;color:white;background:rgba(0, 0, 0, 30%);border:rgba(0, 0, 0, 30%);height:25px;}
        ...
        self.title.setStyleSheet(
            "font:bold 10px;color:white;background:rgba(0, 0, 0, 30%);border:rgba(0, 0, 0, 30%);height:15px;"
        )
    ...
    def _detach_video(self):
        ...
        if txt.lower() == "&detach video":
            ...
            if str(ui.idw) == ui.get_winid() or not ui.idw:
                ...
                if row >= 0:
                    p1 = "ui.gridLayout2.indexOf(ui.label_epn_{0})".format(
                        cur_label_num
                    )
                    index = eval(p1)
                    print(index, "--index--")
                    ...
                    exec(p2)
                    p1 = "ui.float_window_layout.insertWidget(0, ui.label_epn_{0}, 0)".format(
                        cur_label_num
                    )
                    exec(p1)
            ...
            ui.float_window.setGeometry(
                ui.float_window_dim[0],
                ycord,
                ui.float_window_dim[2],
                ui.float_window_dim[3],
            )
            ...
            if ui.list2.count() > 0:
                ...
                if r >= 0:
                    ...
                    try:
                        ...
                        logger.info(
                            "Error in getting Thumbnail: {0}, --line--17659--".format(e)
                        )
        else:
            ...
            if str(ui.idw) == ui.get_winid() or not ui.idw:
                ...
                if cur_label >= 0:
                    p6 = (
                        "ui.gridLayout2.addWidget(ui.label_epn_"
                        + str(cur_label)
                        + ", "
                        + str(r)
                        + ", "
                ...
                if ui.mpvplayer_val.processId() > 0:
                    try:
                        ...
                        p1 = "ui.label_epn_{0}.setTextColor(QtCore.Qt.green)".format(
                            new_cnt
                        )
                        exec(p1)
                        ...
                        try:
                            ...
                            print(e, "--line--4597--")
                            try:
                                p1 = 'ui.label_epn_{0}.setText("{1}")'.format(
                                    new_cnt, txt
                                )
                                exec(p1)
                            except Exception as e:
                                print(e)
                        p1 = "ui.label_epn_{0}.setAlignment(QtCore.Qt.AlignCenter)".format(
                            new_cnt
                        )
                        exec(p1)
                    except Exception as e:
                    ...
                    QtCore.QTimer.singleShot(
                        1000,
                        partial(ui.update_thumbnail_position, context="attach_video"),
                    )
                else:
...
class SystemAppIndicator(QtWidgets.QSystemTrayIcon):
    def __init__(self, parent=None, ui_widget=None, home=None, window=None, logr=None):
        ...
        self.right_menu = RightClickMenuIndicator(
            ui_widget=ui_widget, window=window, logr=logr
        )

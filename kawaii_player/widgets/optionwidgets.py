        ...
        if event.key() == QtCore.Qt.Key_Down:
            ...
            if r > 0:
                ...
                if not ui.video_local_stream:
                    ui.queue_url_list[r], ui.queue_url_list[r1] = (
                        ui.queue_url_list[r1],
                        ui.queue_url_list[r],
                    )
                item = self.item(r)
                txt = item.text()
...
class MySlider(QtWidgets.QSlider):
    def __init__(self, parent, uiwidget, home_dir, mw):
        ...
        self.mpv = MPV(
            vo="image", ytdl="no", quiet=True, aid="no", sid="no", frames=1, idle=True
        )
    ...
    def enterEvent(self, event, source=None):
        ...
        if self.final_url != ui.final_playing_url:
            ...
            if (
                self.file_type == "video"
                and ui.mpvplayer_val.processId() > 0
                and ui.live_preview in ["slow", "fast"]
                or ui.player_val == "libmpv"
            ):
    ...
    def check_and_set_file_type(self):
        if ui.final_playing_url.startswith(
            "abs_path="
        ) or ui.final_playing_url.startswith("relative_path="):
    ...
    def mpv_preview(
        self, preview_dir, t, scale, newpicn, url, final_point=None, timeout=None
    ):
        ...
        if scale is not None:
            ...
            self.mpv.vf = ""
        if not os.path.exists(newpicn) and (
            self.ui.live_preview == "fast"
            or final_point is None
            or self.final_point == final_point
        ):
            self.mpv.play(url)
    ...
    def mouseMoveEvent(self, event, source=None):
        ...
        if (
            ui.live_preview in ["fast", "slow"]
            and (ui.mpvplayer_val.processId() > 0 or ui.player_val in ["libmpv", "mpv"])
            and self.file_type == "video"
            and ui.extra_toolbar_control == "master"
        ):
            if self.preview_dir is None:
                self.create_preview_dir()
            command = 'mpv --vo=image --vo-image-jpeg-quality={} --no-sub --ytdl=no --quiet -aid=no -sid=no --vo-image-outdir="{}" --start={} --frames=1 "{}"'.format(
                ui.live_preview_quality, self.preview_dir, int(t), ui.final_playing_url
            )
            picn = os.path.join(self.preview_dir, "00000001.jpg")
            ...
            if ui.player_val in ["libmpv", "mpv"]:
                ...
                if ui.live_preview == "fast" and not os.path.exists(newpicn):
                    ...
                    if not os.path.exists(newpicn):
                        ...
                        self.preview_thread_list[
                            len(self.preview_thread_list) - 1
                        ].start()
                ...
                if os.path.exists(newpicn):
                    ui.gui_signals.generate_preview(
                        newpicn, l, False, t, 0, event.x(), event.y(), source_val
                    )
        else:
            if self.tooltip is None:
                ...
                point = QtCore.QPoint(
                    ...
                    self.parent.y() + self.parent.height() - offset,
                )
                rect = QtCore.QRect(
                    self.parent.x(),
        ...
        if (
            ...
            and ui.mpvplayer_val.processId() > 0
            and self.file_type == "video"
            and ui.player_val not in ["mpv", "libmpv"]
        ):
            self.setToolTip("")
            ...
            if self.preview_process.processId() == 0 and not use_existing:
                ...
                if ui.live_preview == "fast" and self.preview_process.processId() == 0:
                    ...
                    self.apply_pic(
                        picn, event.x(), event.y(), l, resize=True, only_text=True
                    )
        ...
        if self.preview_process.processId() == 0 and lock is False and not use_existing:
            ...
            if ui.live_preview == "fast":
                ...
                QtCore.QTimer.singleShot(
                    1, partial(self.preview_process.start, command)
                )

    def preview_generated(
        self, picn, length, change_aspect, tsec, counter, x, y, source_val=None
    ):
        # self.tooltip_widget.hide()
        self.preview_counter -= 1
        self.lock = True
    ...
    def apply_pic(
        ...
        if resize:
            txt = '<html><img src="{0}" width="{2}"><p>{1}</p><html>'.format(
                picn, length, 100
            )
        else:
        ...
        if ui.live_preview_style == "tooltip":
            ...
            if self.tooltip is None:
                ...
                if (
                    not self.preview_pending or resize or ui.live_preview == "slow"
                ) and self.enter:
            ...
            try:
                if self.check_dimension_again and not resize:
                    ...
                    self.tooltip_widget.setMaximumSize(
                        QtCore.QSize(img.size[0], img.size[1] + 30)
                    )
                    self.tooltip_widget.setMinimumSize(
                        QtCore.QSize(img.size[0], img.size[1] + 30)
                    )
            ...
            if self.gui.video_local_stream:
                if self.gui.torrent_frame.isHidden():
                    ...
                    if self.gui.torrent_download_limit == 0:
                        ...
                        down_rate = (
                            str(int(self.gui.torrent_download_limit / 1024)) + "K"
                        )
...
class ExtraToolBar(QtWidgets.QFrame):
    def __init__(self, parent, uiwidget):
        ...
        for index, slider in enumerate(slider_list):
            ...
            label_value.returnPressed.connect(
                partial(self.gsbc_entered, label_value, slider)
            )
        ...
        self.btn_aspect_original.clicked_connect(
            partial(self.change_aspect, "original", "btn_aspect_original")
        )
        ...
        self.btn_aspect_disable.clicked_connect(
            partial(self.change_aspect, "disable", "btn_aspect_disable")
        )
        ...
        self.btn_aspect_4_3.clicked_connect(
            partial(self.change_aspect, "4:3", "btn_aspect_4_3")
        )
        ...
        self.btn_aspect_16_9.clicked_connect(
            partial(self.change_aspect, "16:9", "btn_aspect_16_9")
        )
        ...
        self.btn_aspect_235.clicked_connect(
            partial(self.change_aspect, "2.35:1", "btn_aspect_235")
        )
        ...
        self.btn_scr_1.clicked_connect(
            partial(self.execute_command, "async screenshot", "btn_scr_1")
        )
        ...
        self.btn_scr_2.clicked_connect(
            partial(self.execute_command, "async screenshot video", "btn_scr_2")
        )
        ...
        self.btn_scr_3.clicked_connect(
            partial(self.execute_command, "async screenshot window", "btn_scr_3")
        )
        ...
        self.btn_sub_minus.clicked_connect(
            partial(self.execute_command, "add sub-delay -0.1", "btn_sub_minus")
        )
        ...
        self.btn_sub_plus.clicked_connect(
            partial(self.execute_command, "add sub-delay +0.1", "btn_sub_plus")
        )
        ...
        self.btn_aud_minus.clicked_connect(
            partial(self.execute_command, "add audio-delay -0.1", "btn_aud_minus")
        )
        ...
        self.btn_aud_plus.clicked_connect(
            partial(self.execute_command, "add audio-delay +0.1", "btn_aud_plus")
        )
        ...
        self.btn_chapter_minus.clicked_connect(
            partial(self.add_chapter, "-", "btn_chapter_minus")
        )
        ...
        self.btn_chapter_plus.clicked_connect(
            partial(self.add_chapter, "+", "btn_chapter_plus")
        )
        ...
        self.btn_show_stat.setToolTip(
            "<html>Show Some Statistics on Video. Applicable only for mpv v0.28+</html>"
        )
        ...
        self.btn_fs_window.clicked_connect(
            partial(self.execute_command, "TAFS", "btn_fs_window")
        )
        ...
        # self.btn_fs_window.clicked_connect(ui.fullscreenToggle)
        self.btn_fs_video.clicked_connect(
            partial(self.execute_command, "TVFS", "btn_fs_video")
        )
        ...
        self.btn_external_sub.clicked_connect(
            partial(self.execute_command, "external-subtitle", "btn_external_sub")
        )
        ...
        self.subtitle_subscale_value.returnPressed.connect(
            partial(
                self.gsbc_entered, self.subtitle_subscale_value, self.subscale_slider
            )
        )
        ...
        self.scale_label.setToolTip(
            "<html>Scale factor for subtitle. Default 1.0.\
    ...
    def generate_subtitle_frame(self):
        ...
        self.font_label.setToolTip(
            "<html>Set Subtitle Font. Start typing\
        ...
        self.font_value.returnPressed.connect(
            partial(self.change_sub_font, self.font_value, "font_value")
        )
        ...
        self.font_color_value.clicked_connect(
            partial(
                self.choose_color, self.font_color_value, "text", "font_color_value"
            )
        )
        ...
        self.subtitle_text_label.setToolTip(
            "<html>Set Subtitle Text Size. Default, 55</html>"
        )
        ...
        self.subtitle_text_value.returnPressed.connect(
            partial(
                self.gsbc_entered, self.subtitle_text_value, self.subtitle_text_slider
            )
        )

        self.subtitle_border_label = QtWidgets.QLabel(self)
        self.subtitle_border_label.setText("Border")
        self.subtitle_border_label.setToolTip(
            "<html>Set Subtitle Border Size. Default, 3</html>"
        )
        ...
        self.subtitle_border_value.returnPressed.connect(
            partial(
                self.gsbc_entered,
                self.subtitle_border_value,
                self.subtitle_border_slider,
            )
        )

        self.subtitle_shadow_label = QtWidgets.QLabel(self)
        self.subtitle_shadow_label.setText("Shadow")
        self.subtitle_shadow_label.setToolTip(
            "<html>Set Subtitle Shadow Size. Default, 0</html>"
        )
        ...
        self.subtitle_shadow_value.returnPressed.connect(
            partial(
                self.gsbc_entered,
                self.subtitle_shadow_value,
                self.subtitle_shadow_slider,
            )
        )
        ...
        self.subtitle_space_value.returnPressed.connect(
            partial(
                self.gsbc_entered, self.subtitle_space_value, self.subtitle_space_slider
            )
        )

        self.subtitle_blur_label = QtWidgets.QLabel(self)
        self.subtitle_blur_label.setText("Blur")
        self.subtitle_blur_label.setToolTip(
            "<html>Set Gaussian Blur Factor. Default, 0</html>"
        )
        ...
        self.subtitle_blur_value.returnPressed.connect(
            partial(
                self.gsbc_entered, self.subtitle_blur_value, self.subtitle_blur_slider
            )
        )
        ...
        self.subtitle_xmargin_value.returnPressed.connect(
            partial(
                self.gsbc_entered,
                self.subtitle_xmargin_value,
                self.subtitle_xmargin_slider,
            )
        )
        ...
        self.subtitle_ymargin_value.returnPressed.connect(
            partial(
                self.gsbc_entered,
                self.subtitle_ymargin_value,
                self.subtitle_ymargin_slider,
            )
        )
        ...
        self.subtitle_xymargin_value.returnPressed.connect(
            partial(
                self.gsbc_entered,
                self.subtitle_xymargin_value,
                self.subtitle_xymargin_slider,
            )
        )
        ...
        self.subtitle_horiz_align.currentIndexChanged["int"].connect(
            partial(
                ...
                self.subtitle_horiz_align,
                "x",
                "subtitle_horiz_align",
            )
        )
        ...
        self.subtitle_vertical_align.currentIndexChanged["int"].connect(
            partial(
                self.change_alignment,
                self.subtitle_vertical_align,
                "y",
                "subtitle_vertical_align",
            )
        )
        """
        ...
        self.ass_override_value.currentIndexChanged["int"].connect(
            self.ass_override_function
        )

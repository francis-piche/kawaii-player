...
class QtGuiQWidgetScroll(QtWidgets.QScrollArea):
    def __init__(self, parent, uiwidget):
        ...
        self.text_color_focus = ui.thumbnail_text_color_dict[
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_Equal:
            ...
            if iconv_r_poster > 1:
                ...
                width = str(int(w))
                height = str(int(h))
                ui.scrollArea1.verticalScrollBar().setValue(
                    (((ui.cur_row + 1) / iconv_r_poster) - 1) * h
                    + ((ui.cur_row + 1) / iconv_r_poster) * 10
                )
    ...
    def keyPressEvent(self, event):
        if event.key() == QtCore.Qt.Key_Equal:
            ...
            if iconv_r > 1:
                ...
                height = str(int(h))
                ui.scrollArea1.verticalScrollBar().setValue(
                    (((ui.cur_row + 1) / iconv_r) - 1) * h
                    + ((ui.cur_row + 1) / iconv_r) * 10
                )
        elif event.key() == QtCore.Qt.Key_Minus:
            param_dict = ui.get_parameters_value(i="iconv_r", ir="iconv_r_indicator")
            iconv_r = param_dict["iconv_r"]
        ...
        if ui.mpvplayer_val.processId() == 0:
            if event.key() == QtCore.Qt.Key_Right:
                ...
                exec_str = "ui.label_epn_{0}.change_video_mode({1}, {2})".format(
                    num, ui.video_mode_index, num
                )
                exec(exec_str)
            elif event.text().isalnum():
                ...
                if ui.search_on_type_btn.isHidden():
                    ...
                    txt = event.text()
                    ui.search_on_type_btn.setGeometry(
                        g.x() + self.width() - ui.width_allowed,
                        g.y(),
                        ui.width_allowed,
                        32,
                    )

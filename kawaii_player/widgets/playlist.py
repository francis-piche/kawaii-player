...
class PlaylistWidget(QtWidgets.QListWidget):
    ...
    def mouseMoveEvent(self, event):
        if (
            self.ui.auto_hide_dock
            and not self.ui.dockWidget_3.isHidden()
    ...
    def init_offline_mode(self):
        ...
        param_dict = self.ui.get_parameters_value(
            s="site", n="name", sn="siteName", m="mirrorNo"
        )
        ...
        for item in self.selectedItems():
            ...
            if site not in ["Music", "Video"]:
                if (
                    self.ui.wget.processId() == 0
                    and not self.ui.epn_wait_thread.isRunning()
                ):
    ...
    def get_default_name(self, row, mode=None):
        ...
        if site == "Video" and mode == "default":
            ...
            thumbnail_dir = os.path.join(
                self.ui.home_folder, "thumbnails", "thumbnail_server"
            )
            for row, row_item in enumerate(self.ui.epn_arr_list):
    ...
    def move_playlist_items(self, src_row, dest_row, action=None):
        ...
        if (
            (site == "PlayLists" or self.ui.music_playlist or opt == "History")
            and site != "Video"
            and row in range(0, self.count())
        ):
            file_path = ""
            ...
            if os.path.exists(file_path):
                ...
                length = len(lines)
                if (row > 0 and action == "pgup") or (
                    row < length - 1 and action == "pgdown"
                ):
                    if row == length - 1:
                        t = lines[row].replace("\n", "") + "\n"
            ...
            if item:
                if not bookmark:
                    ...
                    cur = conn.cursor()
                    if (row > 0 and action == "pgup") or (
                        row < len(self.ui.epn_arr_list) - 1 and action == "pgdown"
                    ):
                        txt = self.ui.epn_arr_list[row].split("	")[1]
                        ...
                        self.ui.epn_arr_list[row], self.ui.epn_arr_list[nrow] = (
                            self.ui.epn_arr_list[nrow],
                            self.ui.epn_arr_list[row],
                        )
    ...
    def keyPressEvent(self, event):
        if (
            ...
            item, ok = QtWidgets.QInputDialog.getText(
                MainWindow,
                "Input Dialog",
                "Enter Upload Speed in KB\n0 means unlimited",
                QtWidgets.QLineEdit.Normal,
                str(self.ui.setuploadspeed),
            )
            if item and ok:
                if item.isnumeric():
            ...
            if self.ui.list1.isHidden() and self.ui.list1.count() > 0:
                ...
                self.ui.set_parameters_value(
                    show_hide_pl=show_hide_playlist, show_hide_tl=show_hide_titlelist
                )
            ...
            if site == "None":
                ...
                if site == "SubbedAnime" or site == "DubbedAnime":
                    ...
                    if os.path.exists(
                        os.path.join(home, "History", site, name, "Ep.txt")
                    ):
                        file_path = os.path.join(home, "History", site, name, "Ep.txt")
    ...
    def triggerPlaylist(self, value):
        ...
        if (
            ...
            param_dict = self.ui.get_parameters_value(
                f="finalUrlFound", r="refererNeeded"
            )
    ...
    def fix_order(self):
        param_dict = self.ui.get_parameters_value(
            o="opt", s="site", b="bookmark", n="name"
        )
        opt = param_dict["opt"]
        site = param_dict["site"]
        bookmark = param_dict["bookmark"]
    ...
    def setup_slave_address(self, ipaddr=None):
        ...
        if ok and item:
            ...
            msg = "Address of Slave is set to {}, now start media server\
                    and cast single item or playlist".format(
                item
            )
            msg = re.sub(" +", " ", msg)
            send_notification(msg)
                    ...
                    if sub_dict:
                        ...
                        if os.path.isfile(file_path):
                            ...
                            if val:
                                ...
                                self.ui.vnt.post(
                                    item, files=sub_json, timeout=60, verify=verify
                                )
                        else:
    ...
    def start_pc_to_pc_casting(self, mode, row, browser_data=None):
        ...
        if browser_data:
            ...
            ip = "{}://{}:{}/stream_continue.m3u".format(
                http_val, str(self.ui.local_ip_stream), str(self.ui.local_port_stream)
            )
            self.ui.vnt.get(
                ip,
                binary=True,
            ...
            if mode in ["single", "playlist", "queue", "direct url"] and item:
                ...
                for row in range(0, len(new_lines), 2):
                    ...
                    if direct_link or mode == "direct index":
                        ...
                        try:
                            nurl = self.ui.if_path_is_rel(
                                urllib.parse.urlparse(url).path[1:].rsplit("/", 1)[0],
                                abs_path=True,
                                from_master=True,
                            )
                ...
                if val:
                    ...
                    self.ui.vnt.post(
                        ...
                        verify=verify,
                        onfinished=partial(
                            self.final_pc_to_pc_process, pls_file, verify
                        ),
                    )
                else:
                    self.ui.vnt.post(
    ...
    def start_remote_status(self, url):
        ...
        if val:
            self.ui.vnt.get(
                ...
                verify=verify,
                timeout=60,
                onfinished=self.get_remote_status,
            )
        else:
            self.ui.vnt.get(
                nurl, timeout=60, verify=verify, onfinished=self.get_remote_status
            )
    ...
    def get_remote_status(self, *args):
        ...
        if content and self.ui.slave_live_status:
            ...
            if (
                self.ui.extra_toolbar_control == "slave"
                and self.ui.mpvplayer_val.processId() == 0
            ):
    ...
    def contextMenuEvent(self, event):
        ...
        if site == "Music":
            ...
            pls = sorted(
                pls,
                key=lambda x: os.path.getmtime(os.path.join(home_n, x)),
                reverse=True,
            )
            j = 0
            item_m = []
            ...
            if action == new_pls:
                ...
                if not self.ui.float_window.isHidden():
                    if (
                        self.ui.new_tray_widget.cover_mode.text()
                        == self.ui.player_buttons["down"]
                    ):
                ...
                if self.ui.list3.currentItem():
                    ...
                    logger.info(nam1)
                    self.ui.reviewsWeb(
                        srch_txt=nam1, review_site="last.fm", action="search_by_name"
                    )
            elif action == fix_ord:
                self.fix_order()
            ...
            if (
                ...
                if (
                    self.ui.btn1.currentText().lower() == "addons"
            ...
            if goto_web_mode:
                if action == goto_web:
                    self.ui.reviewsWeb(
                        srch_txt=url_web, review_site="YouTube", action="open"
                    )
            if self.ui.pc_to_pc_casting == "master":
                if action == cast_menu_file:
                    self.start_pc_to_pc_casting("single", self.currentRow())
            ...
            if action == new_pls:
                ...
                for item in self.selectedItems():
                    ...
                    self.remove_thumbnails(
                        r, self.ui.epn_arr_list[r], remove_summary=True
                    )

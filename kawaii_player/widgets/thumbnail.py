...
class ThumbnailWidget(QtWidgets.QLabel):
    ...
    def resizeEvent(self, event):
        if self.objectName() == "label_new":
            if (
                not ui.list1.isHidden()
                and not ui.list2.isHidden()
                and not ui.text.isHidden()
            ):
                ...
                self.rect = QtCore.QRect(
                    self.x() + 20, self.y() + 20, self.width() - 40, self.height() - 40
                )
    ...
    def keyReleaseEvent(self, event):
        if (
            ...
            or event.key() == QtCore.Qt.Key_Control
        ):
            ...
            or event.key() == QtCore.Qt.Key_Alt
        ):
            ui.tab_5.event_dict["alt"] = False
            ui.tab_5.event_dict["shift"] = False
        elif (
            event.modifiers() == QtCore.Qt.ShiftModifier
            or event.key() == QtCore.Qt.Key_Shift
        ):
            ui.tab_5.event_dict["shift"] = False
        ...
        if ui.player_val == "mplayer":
            ...
            if site == "Local" or site == "None" or site == "PlayLists":
                ...
                try:
                    ...
                    print(e, "--2309--")
                    logger.info(
                        "seek timer variable not available, creating new seek timer"
                    )
                    self.seek_timer = QtCore.QTimer()
                    self.seek_timer.timeout.connect(self.seek_mplayer)
                    self.seek_timer.setSingleShot(True)
            ...
            if event.key() == QtCore.Qt.Key_Equal:
                ...
                if ui.player_val == "mplayer":
                    if MainWindow.isFullScreen():
                        ...
                        if (
                            site != "Music"
                            and ui.tab_6.isHidden()
                            and ui.list2.isHidden()
                        ):
                            ui.frame1.hide()
    ...
    def player_thumbnail_fs(self, mode=None):
        param_dict = ui.get_parameters_value(icn="iconv_r_indicator", i="iconv_r")
        ...
        if not MainWindow.isHidden():
            if ui.video_mode_index == 6:
                ...
                if not MainWindow.isFullScreen() or mode == "fs" or mode == "fs_now":
                    if mode != "fs":
                        ...
                        print(index, "--index--")
                        if index >= 0:
                            ui.current_thumbnail_position = ui.gridLayout2.getItemPosition(
                                index
                            )
                            ui.tab_6.hide()
            ...
            if not ui.float_window.isHidden():
                if not ui.float_window.isFullScreen():
                    widget = eval(
                        "ui.label_epn_{}".format(ui.thumbnail_label_number[0])
                    )
                    index = ui.gridLayout2.indexOf(widget)
                    print(index, "--index--")
                    if index >= 0:
    ...
    def mouseMoveEvent(self, event):
        ...
        if MainWindow.isFullScreen():
            ht = self.height()
            if (
                not ui.tab_6.isHidden()
                or not ui.list2.isHidden()
                or not ui.list1.isHidden()
                or not ui.tab_2.isHidden()
            ):
                if ui.frame1.isHidden():
                    ui.frame1.show()
            else:
    ...
    def change_video_mode(self, var_mode, c_row, restart=None):
        ...
        param_dict = ui.get_parameters_value(
            ...
            serv="server",
            iconv_r="iconv_r",
            iconv_r_indicator="iconv_r_indicator",
        )
        site = param_dict["site"]
        ...
        if var_mode == 2 or var_mode == 1:
                ...
                if num < ui.list2.count():
                    ...
                    if ui.player_val == "libmpv":
                        ...
                        if ui.gapless_playback:
                            ui.epnfound_now_start_prefetch(
                                finalUrl, num, "thumbnail_gapless"
                            )
                        else:
            ...
            if var_mode == 4:
                ...
                p2 = (
                    "ui.label_epn_"
                    + str(ui.thumbnail_label_number[0])
                ...
                try:
                    ...
                    if site.lower() == "playlists":
                        if ui.is_artist_exists(row):
                            ...
                            try:
                                thumb_path = ui.get_thumbnail_image_path(
                                    row, ui.epn_arr_list[row]
                                )
                                logger.info("thumbnail path = {0}".format(thumb_path))
                                if (
                                    os.path.exists(thumb_path)
                                    and ui.player_theme == "default"
                                ):
                                    ui.videoImage(
                    ...
                    logger.error(
                        "Error in getting Thumbnail--1620-- - localvideogetinlist: {0}".format(
                            e
                        )
                    )
        except Exception as e:
        ...
        try:
            ...
            text_color_focus = ui.thumbnail_text_color_dict[
    ...
    def contextMenuEvent(self, event):
        param_dict = ui.get_parameters_value(
            f="finalUrlFound", r="refererNeeded", i="interval", s="site", n="name"
        )
        ...
        if action == queue_item:
            ...
            if ui.list1.currentItem():
                ...
                nm = ""
            picn = ui.get_thumbnail_image_path(
                num, ui.epn_arr_list[num], only_name=True
            )
            ...
            if (
                ...
                if path.startswith('"'):
                    path = path.replace('"', "", 1)
                picn = picn_new = ui.get_thumbnail_image_path(
                    num, ui.epn_arr_list[num], only_name=True
                )
                ...
                picn = ui.image_fit_option(
                    picn, "", fit_size=6, widget_size=(int(width), int(height))
                )
...
class TitleListWidgetPoster(PlaylistWidget):
    def __init__(
        ...
        super(TitleListWidgetPoster, self).__init__(
            parent, uiwidget, home_var, tmp, logr
        )
    ...
    def keyPressEvent(self, event):
        if event.key() == QtCore.Qt.Key_Return:
            ...
            if (
                self.title_clicked
                and self.currentRow() is not None
                and isinstance(self.currentRow(), int)
            ...
            if ui.search_on_type_btn.isHidden():
                ...
                ui.search_on_type_btn.setGeometry(
                    g.x() + self.width() - ui.width_allowed, g.y(), ui.width_allowed, 32
                )
    ...
    def refill_items(self, mode):
        ...
        for i in range(ui.list1.count()):
            ...
            if os.path.isfile(picn):
                ...
                self.addItem(txt)
            summary = "<html><h1>{0}</h1><head/><body><p>{1}</p></body></html>".format(
                txt, summary
            )
...
class TabThumbnail(QtWidgets.QWidget):
    ...
    def resizeEvent(self, event):
        if ui.tab_6.width() > 500 and not ui.tab_6_player and not ui.lock_process:

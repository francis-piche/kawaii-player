...
class TVDB:
    def __init__(
        ...
        if isinstance(wait, int) or isinstance(wait, float):
            self.vnt = Vinanti(
                block=False, hdrs=self.hdrs, wait=wait, timeout=10, verify=verify
            )
        else:
    ...
    def search_with_backend(self, links, *args):
        if links:
            ...
            if "tab=series" in link:
                new_args = args[:-3]
                self.vnt.head(
                    link, onfinished=partial(self.find_redirected_link, *new_args)
                )
        ...
        if srch.startswith("http"):
            ...
            if backend in ["g", "ddg"]:
                ...
                self.vnt.get(
                    ...
                    params=params,
                    onfinished=partial(
                        self.process_search, onfinished, srch, episode_summary
                    ),
                )
    ...
    def process_page(self, *args):
        ...
        for i, j in series_dict.items():
            self.vnt.get(
                j[0], onfinished=partial(self.process_seasons, args[0], i, ourl, eps)
            )
        return obj
        ...
        if season.lower() == "all seasons" and (self.ep_summary or eps):
            if eps is None or eps is True:
                ...
                for i, val in enumerate(slist):
                    ...
                    if link:
                        logger.debug(
                            "{} {}-----{} {} {}".format(i, obj.total, k, nsid, link)
                        )
                        self.vnt.get(
                            link,
                            onfinished=partial(
                                self.process_episodes, args[0], ourl, val.copy()
                            ),
                        )
                    else:
                        obj.total -= 1

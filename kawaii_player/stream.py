    ...
    def get_the_content(self, get_bytes):
        ...
        if not os.path.exists(file_name):
            ...
            if (
                ...
                and os.stat(file_name).st_size == content_length
            ):
                complete_file = True
        if get_bytes:
        ...
        if get_bytes or upper_range is not None:
            ...
            print(
                "...sending range...{0}-{1}/{2}".format(
                    str(get_bytes), str(upper_range), str(content_length)
                )
            )
            self.send_header(
                "Content-Range",
            ...
            while content and req_piece in range(count, count_limit + 1):
                ...
                if handle.have_piece(req_piece):
                    ...
                    for piece in range(req_piece, req_piece + 7):
                        if piece in range(
                            count, count_limit + 1
                        ) and not handle.have_piece(piece):
                            if priority in [1, 2]:
                    ...
                    if req_piece in range(count_limit - 10, count_limit - 7):
                        for k in range(count + 1, req_piece - 1):
                            if (
                                not handle.have_piece(k)
                                and handle.piece_priority(k) == 0
                            ):
                    ...
                    if get_bytes and not pri_lowered:
                        if seek_end:
                            ...
                            ncount = count + 1
                        print(
                            req_piece - 1, ncount, "----", count_limit, count_limit - 1
                        )
                        if req_piece not in [count_limit, count_limit - 1]:
                            for k in range(ncount, req_piece - 1):
                                if not handle.have_piece(k) and handle.piece_priority(
                                    k
                                ) in [1, 2]:
                                    handle.piece_priority(k, 0)
                                    print(
                                        "lowered {} {}/{}".format(k, count, count_limit)
                                    )
                        pri_lowered = True
                        self.show_piece_map(info, handle, req_piece)
                if update_str and not ui_player.torrent_frame.isHidden():
                    ...
                    if ui_player.torrent_status_command == "map":
                        update_str = self.show_piece_map(
                            info, handle, req_piece, local=True
                        )
        ...
        if "&pl_id=" in self.path:
            ...
            try:
                if pl_id in ui_player.playlist_auth_dict_ui:
                    print(
                        pl_id,
                        ui_player.playlist_auth_dict_ui[pl_id],
                        "--playlist--id--",
                    )

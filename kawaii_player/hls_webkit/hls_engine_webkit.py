...
from netmon_webkit import NetManager
...
class BrowserPage(QWebPage):
    ...
    def _writeCookies(self, i):
        ...
        # print(reqkey, self.domain_name)
        if dm:
            str1 = ""
            if reqkey:
                name_id_key = reqkey[reqkey["name_id"]]
                if name_id_key not in self.cookie_dict:
                    str1 = (
                        reqkey["domain"]
                        + "	"
                        + "FALSE"
                        ...
                        + reqkey["name_id"]
                        + "	"
                        + reqkey[reqkey["name_id"]]
                    )
                    self.cookie_dict.update({name_id_key: str1})
                    self.cookie_string = self.cookie_string + str1 + "\n"
            if self.set_cookie is not None and str1:
                cc = self.set_cookie
    ...
    def _loadProgress(self):
        ...
        if self.set_cookie:
            self._cookie(self.set_cookie)
        if self.js_progress:
            val = frame.evaluateJavaScript(self.js_content)
            self.val_progress(val)
...
class BrowseUrlT(QWebView):
    ...
    def __init__(
        ...
        self.set_cookie = set_cookie
        self.use_cookie = use_cookie
        print(self.set_cookie, self.use_cookie)
        self.value_encode = ""
        if end_point:
            self.end_point = end_point
        else:
    ...
    def Browse(self, url):
        ...
        self.link_signal.connect(self.link_source_found)
        self._set_cookie(url)
        self.web.settings().setAttribute(QWebSettings.LocalStorageEnabled, True)
        self.web.settings().enablePersistentStorage(self.tmp_dir)

...
def ccurl_string_get(url, opt, extra, download_manager=None):
    ...
    if len(ver_peer) > 3:
        ...
        if ver_peer_get.startswith("abs_path=") or ver_peer_get.startswith(
            "master_abs_path="
        ):
            if "-k" not in command:
                command.append("-k")
    ...
    if curl_opt == "-o":
        ...
        try:
            ...
            if len(ver_peer) > 3 and os.name != "nt":
                ver_peer_get = ver_peer[3]
                if (
                    ver_peer_get.startswith("abs_path")
                    and "&pl_id=" in ver_peer_get
                    and verify_peer is not False
                ):
                    c.setopt(c.SSL_VERIFYPEER, False)
            c.perform()
            c.close()

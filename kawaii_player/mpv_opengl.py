...
class QProcessExtra(QtCore.QProcess):
    ...
    def write(self, cmd):
        if (
            self.ui.player_val == "libmpv"
            and self.ui.tab_5.mpv.get_property("idle-active") is False
        ):
            print(cmd)
            ...
            if cmd_arr[0] == "show-text":
                ...
                if "filename" in cmd_tail:
                    ...
                    if "pause" in cmd or "2000" in cmd:
                        try:
                            ...
                            time_pos = 0
                        display_string = self.ui.tab_5.display_play_pause_string(
                            time_pos
                        )
                        if self.ui.tab_5.mpv_api == "opengl-render":
                            cmd_tail = (
                                self.ui.tab_5.mpv.get_property("osd-sym-cc")
                                + " "
                                + display_string
                            )
                        else:
                            cmd_tail = (
                                self.ui.tab_5.mpv.osd_sym_cc
                                + b" "
                                + bytes(display_string, "utf-8")
                            )
                    else:
                        cmd_list = cmd.split()
                        cmd_list = cmd_list[1:]
                ...
                self.ui.tab_5.mpv.command(
                    "show-text", "Adding-subtitle: {}".format(sub_file), 2000
                )
            elif cmd_arr[0] == "loadfile":
                file_path = self.filter_command(cmd)
                self.ui.tab_5.mpv.command(
                    "show-text", "loading: {}".format(file_path), 2000
                )
            else:
                try:
                    ...
                    print(e)
                    self.ui.tab_5.mpv.command(
                        "show-text", "not found: {}, {}".format(cmd, e), 5000
                    )
        elif (
            self.ui.player_val == "libmpv"
            and self.ui.tab_5.mpv.get_property("idle-active") is True
        ):
...
class MpvOpenglWidget(QOpenGLWidget):
    def __init__(
        self, parent=None, ui=None, logr=None, tmp=None, mpv_api=None, app=None
    ):
        ...
        if platform.system().lower() == "darwin":
            ...
            if (
                hasattr(self.ui, "desktop_session")
                and self.ui.desktop_session == "lxde-pi"
            ):
        ...
        self.modifiers = set(
            [QtCore.Qt.ShiftModifier, QtCore.Qt.ControlModifier, QtCore.Qt.AltModifier]
        )
    ...
    def init_opengl_render(self):
        ...
        for key, value in self.args_dict.items():
            ...
            try:
                ...
                logger.error(
                    "Error in setting property: {} => {}, Correct config options.".format(
                        key, value
                    )
                )
            if key == "msg-level":
        ...
        while self.mpv:
            try:
                ...
                if event.id in {mpv.Events.none, mpv.Events.shutdown}:
                    ...
                    log_msg = "[{}] {}-{}".format(
                        event_log.level, event_log.prefix, event_log.text.strip()
                    )
                    print(log_msg)
    ...
    def init_mpv_again(self):
        if self.mpv_api in [
            "opengl-cb",
            "opengl-render",
        ] and platform.system().lower() in ["linux", "windows"]:
    ...
    def arrow_hide(self):
        if gui.player_val in ["mpv", "mplayer", "libmpv"]:
            ...
            if (
                self.ui.fullscreen_video
                and self.hasFocus()
                and self.ui.tab_6.isHidden()
                and self.ui.list2.isHidden()
                and self.ui.tab_2.isHidden()
            ):
                self.ui.frame1.hide()
    ...
    def quit_watch_later(self, _name, value):
        ...
        if value is True:
            self.rem_properties(
                ...
                rem_quit=1,
                seek_time=self.ui.progress_counter,
            )
    ...
    def display_play_pause_string(self, value):
        ...
        if value is not None:
            ...
            if not cache:
                cache = 0
            if gui.mplayerLength > 0:
                display_string = "{}%  {} / {}  Cache: {}s".format(
                    percent, fn(value), fn(gui.mplayerLength), int(cache)
                )
    ...
    def playlist_position_observer(self, _name, value):
        ...
        if isinstance(value, int) and value >= 0:
            cur_row = value
            if (
                self.mpv.get_property("playlist-count") > 1
                and cur_row < gui.list2.count()
                ...
                if (
                    gui.view_mode == "thumbnail_light"
                    and gui.cur_row < gui.list_poster.count()
                ):
            ...
            if gui.gapless_network_stream and not gui.queue_url_list:
                if gui.progress_counter > int(gui.mplayerLength / 2):
                    if (gui.cur_row + 1) < gui.list2.count():
                        ...
                        item_index = 0
                    if (
                        gui.tmp_pls_file_dict.get(item_index) is False
                        and gui.list2.count() > 1
                    ):
                        gui.start_gapless_stream_process(item_index)
            if (
                self.prefetch_url
                and isinstance(self.prefetch_url, tuple)
                and gui.gapless_network_stream
            ):
        ...
        self.ui.history_dict_obj_libmpv.update(
            {
                final_url: [
                    ...
                    rem_quit,
                    self.player_volume,
                    self.ui.mpvplayer_aspect_cycle,
                ]
            }
        )
        logger.info(
            "remember-- {}".format(
                self.ui.history_dict_obj_libmpv.get(self.ui.final_playing_url)
            )
        )
    ...
    def eof_observer(self, _name, value):
        ...
        if value is True or value is None:
            ...
            if self.ui.queue_url_list:
                ...
                self.playlist_backup = False
        if value in [None, True] and (
            self.ui.quit_really == "yes" or not self.ui.playlist_continue
        ):
            ...
            if "\t" in ourl:
                ...
                if ourl.startswith("http"):
                    ...
                    if param:
                        ...
                        ext_list = list(
                            itertools.chain.from_iterable(list(map(ext_fn, slang_list)))
                        )
                        for ext in ext_list:
                            ...
                            sub_arr.append(
                                {
                                    "path": os.path.join(
                                        self.ui.yt_sub_folder, param + "." + ext
                                    ),
                                    "type": file_type,
                                    "lang": lang,
                                }
    ...
    def idle_observer(self, _name, value):
        site = self.ui.get_parameters_value(st="site")["site"]
        logger.debug(
            "...{}={}, quit-now={} site={}".format(value, _name, gui.quit_now, site)
        )
        if (
            value is True
            and self.started
    ...
    def time_duration(self, _name, value):
        if value is not None:
            ...
            if not self.initial_volume_set:
                logger.debug(
                    "setting..........volume.......{}".format(gui.player_volume)
                )
                try:
            ...
            if saved_url in self.ui.history_dict_obj_libmpv:
                (
                    ...
                    vol,
                    asp,
                ) = self.ui.history_dict_obj_libmpv.get(saved_url)
                ...
                if aspect_val and gui.restore_aspect:
                    ...
                    self.mpv.command(
                        "show-text", "Bad Aspect Property: {}".format(aspect_val)
                    )
                try:
            ...
            if (
                self.ui.pc_to_pc_casting == "slave"
                and "master_abs_path=" in self.ui.final_playing_url
            ):
    ...
    def change_aspect_ratio(self, key=None):
        ...
        aspect_val = self.ui.mpvplayer_aspect_float.get(
            str(self.ui.mpvplayer_aspect_cycle)
        )
        self.mpv.set_property("video-aspect", aspect_val)
        ...
        if " " in cmd:
            ...
            if cmd_name in ["volume", "ao-volume"]:
                ...
                try:
                    ...
                    slider = eval(
                        "self.ui.frame_extra_toolbar.{}_slider".format(cmd_name)
                    )
            ...
            if self.player_val == "libmpv":
                if mode == "fs":
                    ...
                    if self.ui.orientation_dock == "right":
                        self.ui.superGridLayout.addWidget(
                            self.ui.dockWidget_3, 0, 1, 1, 1
                        )
                    self.show()
                    self.setFocus()
                    ...
                    if (
                        ...
                        and self.ui.osx_native_fullscreen
                    ):
                        MainWindow.hide()
                        self.setParent(None)
            ...
            if not MainWindow.isHidden():
                if self.ui.video_mode_index in [5, 6, 7]:
                    ...
                    if not MainWindow.isFullScreen():
                        ...
                        logger.debug(index)
                        if index >= 0:
                            self.ui.current_thumbnail_position = self.ui.gridLayout2.getItemPosition(
                                index
                            )
                            self.ui.tab_6.hide()
                            self.ui.gridLayout.addWidget(widget, 0, 1, 1, 1)
                            widget.setMaximumSize(
                                QtCore.QSize(screen_width, screen_height)
                            )
                            self.ui.gridLayout.setContentsMargins(0, 0, 0, 0)
                            self.ui.superGridLayout.setContentsMargins(0, 0, 0, 0)
                        ...
                        cur_label = cur_label_num
                        self.ui.gridLayout2.addWidget(
                            widget, r, c, 1, 1, QtCore.Qt.AlignCenter
                        )
                        ...
                        QtCore.QTimer.singleShot(
                            1000, self.ui.update_thumbnail_position
                        )
                        self.ui.fullscreen_video = False
            else:
                if not self.ui.float_window.isHidden():

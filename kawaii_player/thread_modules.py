...
class FindPosterThread(QtCore.QThread):
    ...
    def parse_tmdb(self, name, final_link, thumb, fanart):
        ...
        content = ccurl(url)
        bcontent = ccurl(burl)
        posters_link = re.findall(
            'https://image.tmdb.org/[^"]*original[^"]*.jpg', content
        )
        fanart_link = re.findall(
            'https://image.tmdb.org/[^"]*original[^"]*.jpg', bcontent
        )
        if not fanart_link:
    ...
    def init_search(self, nam, url, direct_url, thumb, fanart, src_site):
        ...
        if self.use_search:
            if isinstance(self.use_search, bool):
                ...
                final_link, m = ui.metaengine.ddg_search(
                    nam, self.use_search, direct_search=True
                )
                src_site = self.use_search
        else:
            if direct_url and url:
                if (
                    ".jpg" in url or ".png" in url or url.endswith(".webp")
                ) and "http" in url:
                    if self.copy_poster:
                        ccurl(url + "#" + "-o" + "#" + thumb)
                ...
                link = (
                    "http://thetvdb.com/index.php?seriesname="
                    + nam
                    + "&fieldlocation=1&language=7&genre=Animation&year=&network=&zap2it_id=&tvcom_id=&imdb_id=&order=translation&addedBy=&searching=Search&tab=advancedsearch"
                )
                logger.info(link)
                content = ccurl(link)
                ...
                if not m:
                    link = (
                        "http://thetvdb.com/index.php?seriesname="
                        + nam
                        + "&fieldlocation=2&language=7&genre=Animation&year=&network=&zap2it_id=&tvcom_id=&imdb_id=&order=translation&addedBy=&searching=Search&tab=advancedsearch"
                    )
                    content = ccurl(link)
                    m = re.findall('/index.php[^"]tab=[^"]*', content)
                    if not m:
                        link = (
                            "http://thetvdb.com/?string="
                            + nam
                            + "&searchseriesid=&tab=listseries&function=Search"
                        )
                        content = ccurl(link)
        ...
        if site == "Music":
            ...
            if site.lower() == "video" and self.video_dir:
                ...
                if os.path.exists(video_db):
                    epn_arr_tmp = ui.media_data.get_video_db(
                        video_db, "Directory", self.video_dir
                    )
                ...
                if (
                    site.lower() == "subbedanime"
                    or site.lower() == "dubbedanime"
                    and siteName
                ):
                    hist_site = os.path.join(
                        ui.home_folder, "History", site, siteName, name_title
                    )
                else:
                    hist_site = os.path.join(
                        ui.home_folder, "History", site, name_title
                    )
            ...
            if (
                (m and src_site in ["tvdb", "tvdb+g", "tvdb+ddg"])
                or post_val
                or fan_val
            ):
...
def update_image_list_method(image_dict, dest_dir, site):
    ...
    if (
        ...
        for img_key, img_val in image_dict.items():
            ...
            ui.downloadWget.append(DownloadThread(ui, picn_url, img_list=img_val_copy))
            ...
            ui.downloadWgetText.append(
                DownloadThread(ui, picn_url, img_list=img_val_copy, get_text=True)
            )
            length_text = len(ui.downloadWgetText)
            ui.downloadWgetText[length_text - 1].finished.connect(
                partial(
                    ui.download_thread_text_finished,
                    dest_txt,
                    txt_count,
                    length_text - 1,
                )
...
class YTdlThread(QtCore.QThread):
    ...
    def run(self):
        final_url = ""
        try:
            final_url = ui.yt.get_yt_url(
                self.url, self.quality, self.path, self.loger, mode="a+v"
            )
...
class UpdateMusicThread(QtCore.QThread):
    ...
    def run(self):
        ...
        ui.media_data.update_on_start_music_db(
            self.music_db, self.music_file, self.music_file_bak
        )
        ...
        if not self.get_text:
            ...
            try:
                if self.url:
                    ...
                    ui.image_fit_option(
                        self.picn, self.picn, fit_size=6, widget=ui.label
                    )
                    if site.lower() == "video":
...
class PlayerGetEpn(QtCore.QThread):
    ...
    def __init__(self, ui_widget, logr, epn_type, *args):
                ...
                if self.epn_type == "type_three":
                    ...
                    if self.site_name != self.site:
                        plugin_path = os.path.join(
                            ui.home_folder, "src", "Plugins", self.site + ".py"
                        )
                        if os.path.exists(plugin_path):
                            module = imp.import_module(self.site, plugin_path)
                            self.site_var = getattr(module, self.site)(
        ...
        self.get_offline_signal_list_type_three.connect(
            connect_to_offline_mode_list_type_three
        )
    ...
    def run(self):
        ...
        try:
            ...
            if self.epn_type == "yt":
                finalUrl = ui.yt.get_yt_url(
                    self.final, self.quality, self.yt_path, logger, mode="a+v"
                )
            elif self.epn_type == "yt_music":
                finalUrl = ui.yt.get_yt_url(
                    self.final, self.quality, self.yt_path, logger, mode="music"
                )
                ...
                if self.final.startswith("http"):
                    finalUrl = ui.yt.get_yt_url(
                        self.final, self.quality, self.yt_path, logger, mode=ytmode
                    )
                else:
                    finalUrl = self.final
                ...
                finalUrl = ui.yt.get_yt_url(
                    self.final, self.quality, self.yt_path, logger, mode="TITLE"
                )
                ...
                finalUrl = ui.site_var.getFinalUrl(
                    self.siteName, self.name, self.epn, self.mirrorNo, self.quality
                )
            elif self.epn_type == "offline":
                finalUrl = ui.epn_return(self.row, mode=self.mode)
                logger.debug(finalUrl)
            elif self.epn_type == "list":
                mytuple = ui.site_var.getEpnList(
                    self.name,
                    self.opt,
                    self.depth_list,
                    self.extra_info,
            ...
            if not isinstance(finalUrl, list):
                if self.epn_type == "offline":
                    ...
                    if ui.player_val == "libmpv":
                        ...
                        self.final_epn_prefetch_signal.emit(
                            finalUrl, self.row, self.epn_type
                        )
                else:
        ...
        if not os.path.exists(self.picn) and self.path:
            try:
                if self.path.startswith("http") and (
                    ...
                    ui.generate_thumbnail_method(
                        self.picn, self.inter, self.path, width_allowed=self.size
                    )
            except Exception as e:
...
def apply_to_thumbnail_grid(
    browse_cnt, picn, val, fit_size, widget_size, length, nameEpn
):
    try:
        picn_old = picn
        picn = ui.image_fit_option(
            picn_old, "", fit_size=fit_size, widget_size=widget_size
        )
...
class DiscoverServer(QtCore.QThread):
    ...
    def run(self):
        ...
        while ui.discover_server or ui.discover_slaves:
            try:
                ...
                if val.lower().startswith("this is kawaii-player at:"):
                    ...
                    if (
                        server_val not in ui.broadcast_server_list
                        and ui.discover_server
                    ):
        ...
        if not ui.broadcast_server_list and ui.discover_server:
            ...
            send_notification(
                "Stopped Discovering: found {} servers".format(
                    len(ui.broadcast_server_list)
                )
            )
        elif ui.pc_to_pc_casting_slave_list:
            send_notification(
                "Stopped Discovering: found {} slaves".format(
                    len(ui.pc_to_pc_casting_slave_list)
                )
            )

...
class Browser(QtWebEngineWidgets.QWebEngineView):
    ...
    def final_found(self, final_url):
        if final_url:
            self.ui.logger.debug(final_url)
    ...
    def var_remove(self, var):
        self.ui.logger.debug("{} ----js----- ".format(var))
        ...
        if prev_url != link.url() and "youtube.com" in link.url():
            ...
            if "/watch?" in link.url():
                ...
                try:
                    ...
                    self.ui.logger.error(err)
            if (
                self.current_link.startswith("https://m.youtube.com/watch?v=")
                or self.current_link.startswith("https://www.youtube.com/watch?v=")
    ...
    def yt_process_started(self):
        self.ui.logger.debug("yt_process_started")
    ...
    def clicked_link(self, link):
        ...
        self.ui.logger.info(
            "--clicked--link: {0}--{1}".format(url, self.epn_name_in_list)
        )
        ...
        if url or self.media_url:
            if url:
                if "tvdb" in url:
                    ...
                    "youtube.com" in url or "ytimg.com" in url or url.startswith("http")
                ):
                    yt = True
            ...
            if "youtube.com/watch?v=" in self.url().url() or self.url().url().startswith(
                "http"
            ):
                self.title_page = self.title()
                if (
                    self.title_page
                    and self.title_page.lower() == "youtube"
                ...
                "tvdb" in self.url().url()
                or "last.fm" in self.url().url()
                or "themoviedb" in self.url().url()
                ...
                for i, acts in enumerate(action):
                    if act == acts:
                        self.download(
                            self.url().url(), arr[i], copy_summary=self.selected_text
                        )
            else:
    ...
    def download(self, url, option, copy_summary=None):
        if option.lower() == "play with kawaii-player":
            ...
            finalUrl = self.ui.yt.get_yt_url(
                url, quality, self.ui.ytdl_path, self.ui.logger, mode="offline"
            )

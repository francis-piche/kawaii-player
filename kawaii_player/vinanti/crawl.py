...
class CrawlObject:
    def __init__(
        self, vnt, url_obj, onfinished, all_domain, domains_allowed, depth_allowed
    ):
        url = url_obj.url
            ...
            if self.depth_allowed > depth or self.depth_allowed <= 0:
                ...
                for links in link_list:
                    for link in links:
                        ...
                        if not lnk or lnk == "#":
                            continue
                        lnk = self.construct_link(
                            ourl, scheme, netloc, url, base_url, lnk
                        )
                        if lnk:
                            self.crawl_next_link(
                                lnk, session, base_url, depth, result.out_dir
                            )
    ...
    def construct_link(self, ourl, scheme, netloc, url, base_url, lnk):
        if lnk and "#" in lnk:

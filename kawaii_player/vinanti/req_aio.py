...
class RequestObjectAiohttp(RequestObject):
    ...
    async def process_aio_request(self, session):
        ...
        async with func as resp:
            rsp = Response(
                self.url, method=self.method, out_file=self.out, out_dir=self.out_dir
            )
                        ...
                        with open(self.out, mode) as fd:
                            while True:
                                ...
                                if (print_count) % 200 == 0:
                                    ...
                                    sys.stdout.write("\r")
                                    sys.stdout.write(
                                        "{} M / {} M : {}".format(dwn, sz, self.out)
                                    )
                                    sys.stdout.flush()
                            sys.stdout.write("\r")
                            sys.stdout.write(
                                "{} M / {} M : {}".format(sz, sz, self.out)
                            )
                            sys.stdout.flush()
    ...
    def add_formfields(self):
        ...
        if isinstance(self.files, str):
            ...
            self.data.add_field(
                filename, open(self.files, "rb"), content_type=content_type
            )
            ...
            for file_name in self.files:
                ...
                self.data.add_field(
                    filename, open(file_name, "rb"), content_type=content_type
                )
        elif isinstance(self.files, dict):
            for file_title, file_name in self.files.items():
                content_type = self.get_content_type(file_name)
                self.data.add_field(
                    file_title, open(file_name, "rb"), content_type=content_type
                )

...
class Vinanti:
    def __init__(
        ...
        **kargs
        ...
        logger.info(
            "multiprocess: {}; max_requests={}; backend={}".format(
    ...
    def __build_tasks__(
        self, urls, method, onfinished=None, hdrs=None, options_dict=None
    ):
        self.tasks.clear()
        ...
        if self.session_params:
            ...
            method, onfinished, hdrs, options_dict = self.__set_session_params__(
                *global_params
            )
        ...
        if self.block:
            ...
            if isinstance(urls, str):
                ...
                session, netloc = self.__request_preprocess__(
                    urls, hdrs, method, options_dict
                )
    ...
    def add(self, urls, onfinished=None, hdrs=None, method=None, **kargs):
        if self.session_params:
            ...
            method, onfinished, hdrs, kargs = self.__set_session_params__(
                *global_params
            )
        if isinstance(urls, str):
            length = len(self.tasks)
    ...
    def start(self, task_dict=None, queue=False):
        ...
        if (not self.loop and task_dict) or (task_dict and self.old_method):
            ...
            if self.old_method:
                loop_thread = Thread(
                    target=self.__start_non_block_loop_old__, args=(task_dict, loop)
                )
            else:
                loop_thread = Thread(
                    target=self.__start_non_block_loop__, args=(task_dict, self.loop)
                )
    ...
    def __finished_task_postprocess__(
        self,
        session,
        netloc,
        onfinished,
            ...
            if self.old_method:
                ...
                for task_list in self.task_queue:
                    self.loop.create_task(
                        self.__start_fetching__(*task_list, self.loop)
                    )
                self.task_queue.clear()
                logger.info(
        ...
        if onfinished:
            logger.info("arranging callback, task {} {}".format(task_num, url))
            onfinished(task_num, url, result)
            logger.info("callback completed, task {} {}".format(task_num, url))
    ...
    async def __start_fetching__(
        self, url_obj, onfinished, hdrs, method, kargs, task_num, loop
    ):
        async with self.sem:
            ...
            if isinstance(kargs, dict):
                ...
                if mp:
                    ...
                    logger.info(
                        "using multiprocess with max_workers={}".format(workers)
                    )
                else:
            ...
            if backend == "urllib" and isinstance(url, str):
                logger.info("\nRequesting url: {}\n".format(url))
                session, netloc = await self.__request_preprocess_aio__(
                    url, hdrs, method, kargs
                )
                future = loop.run_in_executor(
                    executor, get_request, backend, url, hdrs, method, kargs
                )
                ...
                async with aio:
                    try:
                        response = await self.__fetch_aio__(
                            url, aio, hdrs, method, kargs
                        )
                ...
                future = loop.run_in_executor(
                    executor, complete_function_request, url, kargs
                )
                response = await future

            self.__finished_task_postprocess__(
                session,
                netloc,
                onfinished,
                task_num,

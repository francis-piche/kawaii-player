...
class LoginAuth(QtWidgets.QDialog):
    def __init__(
        ...
        if settings:
            ...
            self.backg.setToolTip(
                "yes:Keep same default background for all\nno:keep changing the background as per title"
            )
            ...
            self.img_opt.setToolTip(
                "Use Ctrl+0 to Ctrl+9 keyboard shortcuts to Experiment with various background image modes. \n1:Fit To Screen\n2:Fit To Width\n3:Fit To Height\n4:Fit Upto Playlist\nRestart to see the effect OR if want to see immediate effect, then directly use keyboard shortcuts"
            )
            img_opt_arr = [
                "IMAGE FIT OPTIONS",
                "1",
    ...
    def _set_params(self):
        ...
        try:
            ...
            if ipaddress.ip_address(new_ip_val):
                ...
                torrent_ip = (
                    "TORRENT_STREAM_IP=" + new_ip_val + ":" + str(self.ui.local_port)
                )
        ...
        if torrent_ip:
            config_file_torrent = os.path.join(
                self.ui.home_folder, "torrent_config.txt"
            )
    ...
    def handleLogin(self):
        ...
        if (
            not content
            or "www-authenticate" in content.lower()
            or "401 unauthorized" in content.lower()
            or "curl failure" in content.lower()
        ) and self.count < 3:
...
class LoginPCToPC(QtWidgets.QDialog):
    ...
    def handle_login(self):
        ...
        self.ui.vnt.post(
            ...
            files=self.pls_file,
            onfinished=partial(
                self.post_login, text_val, pass_val, self.url, self.onfinished
            ),
            timeout=10,
            session=True,
            verify=self.verify,
...
class OptionsSettings(QtWidgets.QTabWidget):
    ...
    def appeareance_setdefault(self):
        ...
        QtCore.QTimer.singleShot(
            2000, partial(self.apply_changes_to_file, self.option_file)
        )
    ...
    def apply_changes_to_file(self, file_name):
        ...
        for i in lines:
            ...
            if i.startswith("THEME="):
                ...
                i = "THUMBNAIL_TEXT_COLOR_FOCUS={}".format(
                    ui.thumbnail_text_color_focus
                )
    ...
    def appeareance(self):
        ...
        for i, j in enumerate(self.param_list):
            ...
            if isinstance(line, QtWidgets.QComboBox):
                line.currentIndexChanged["int"].connect(
                    partial(self.combobox_changed, line, j, "appearance")
                )
            elif isinstance(line, QtWidgets.QLineEdit):
                line.returnPressed.connect(
                    partial(self.line_entered, line, j, "appearance")
                )

        self.bg_color_label = QtWidgets.QLabel(self)
        self.bg_color_label.setText("BG Color")
        self.bg_color_label.setToolTip(
            "<html>Set Background Color. Only works in dark theme.</html>"
        )
        ...
        self.bg_color_label_widgetc.setToolTip(
            "<html>Set Control Frame Color. Only works in dark theme.</html>"
        )
        ...
        self.bg_color_value_widgetc.setToolTip(
            "<html>click to change background color of control frame</html>"
        )
        self.bg_color_value_widgetc.clicked_connect(
            partial(
                self.choose_color,
                self.bg_color_value_widgetc,
                "bg-widget",
                "bg_color_value_widgetc",
            )
        )
        ...
        self.bg_color_value_widgetcd.setToolTip(
            "<html>Restore Default Color of Control Frame</html>"
        )
    ...
    def restore_color(self, widget, widget_type):
        if widget_type == "frame":
            ...
            change_opt_file(
                ...
                "BG_COLOR_CONTROL_FRAME=" + str(rgb),
            )
        elif widget_type == "bg":
            rgb = (56, 60, 74)
    ...
    def set_colors(self, widget, widget_name, rgb):
        ...
        if widget_name == "bg_color_value":
            ...
            change_opt_file(
                self.option_file,
                "BG_COLOR_WIDGET_DARK_THEME=",
                "BG_COLOR_WIDGET_DARK_THEME=" + str(rgb),
            )
        elif widget_name == "bg_color_value_widgetc":
            ui.bg_color_control_frame = rgb
            ui.widget_style.apply_stylesheet(theme=ui.player_theme)
    ...
    def mediaserver(self):
        ...
        self.btn11.clicked_connect(
            partial(self.check_and_set_ip, self.line11, "local_ip_stream")
        )
        ...
        self.line15.setPlaceholderText(
            "{} (In Hours)".format(ui.cookie_playlist_expiry_limit)
        )
        ...
        self.line17.setPlaceholderText(
            "{} (upload speed in peer to peer mode)".format(ui.setuploadspeed)
        )
        ...
        for index, j in enumerate(self.media_param):
            ...
            if isinstance(line, QtWidgets.QComboBox):
                line.currentIndexChanged["int"].connect(
                    partial(self.combobox_changed, line, j, "mediaserver")
                )
            elif isinstance(line, QtWidgets.QLineEdit):
                line.returnPressed.connect(
                    partial(self.line_entered, line, j, "mediaserver")
                )
    ...
    def mediaserver_extra_buttons(self, index=None, mode=None):
        if index and mode == 1:
            ...
            self.line021.clicked_connect(
                partial(self.media_server_start, "broadcast server")
            )
            ...
            self.line020.clicked_connect(
                partial(self.media_server_start, "remote_control")
            )
    ...
    def torrentsettings(self):
        ...
        self.btn21.clicked_connect(
            partial(self.check_and_set_ip, self.line21, "local_ip")
        )
        ...
        self.btn22.clicked_connect(
            partial(self.set_folder, self.line22, "torrent_download_folder")
        )
        self.torrent_param.append("torrent_download_folder")

        self.line23 = QtWidgets.QLineEdit()
        self.line23.setPlaceholderText(
            "{} (in KB, 0 means unlimited )".format(int(ui.torrent_upload_limit / 1024))
        )
        ...
        self.line24.setPlaceholderText(
            "{}".format(int(ui.torrent_download_limit / 1024))
        )
        ...
        for index, j in enumerate(self.torrent_param):
            ...
            if isinstance(line, QtWidgets.QComboBox):
                line.currentIndexChanged["int"].connect(
                    partial(self.combobox_changed, line, j, option="torrent")
                )
            elif isinstance(line, QtWidgets.QLineEdit):
                line.returnPressed.connect(
                    partial(self.line_entered, line, j, option="torrent")
                )
    ...
    def othersettings(self):
        ...
        self.btn31.clicked_connect(
            partial(self.set_folder, self.line31, "default_download_location")
        )
        ...
        self.text33.setToolTip(
            "<html>default, automatic or complete path.\
        ...
        for index, j in enumerate(self.other_settings):
            ...
            if isinstance(line, QtWidgets.QComboBox):
                line.currentIndexChanged["int"].connect(
                    partial(self.combobox_changed, line, j, "othersettings")
                )
            elif isinstance(line, QtWidgets.QLineEdit):
                line.returnPressed.connect(
                    partial(self.line_entered, line, j, "othersettings")
                )
    ...
    def player_settings(self):
        ...
        self.btn45.clicked_connect(
            partial(self.set_folder, self.line45, "screenshot_directory")
        )
        ...
        self.line49_2.returnPressed.connect(
            partial(
                self.line_entered,
                self.line49_2,
        ...
        self.line49_2.setToolTip(
            "<html>Set Seekbar Live preview thumbnail image quality (0-100). Default is 90. Lower quality will generate faster preview at the expense of image quality. Higher value (i.e. Higher quality) will generate preview images slowly depending upon cpu speed</html>"
        )
        ...
        for i, j in enumerate(self.player_list):
            ...
            if isinstance(line, QtWidgets.QComboBox):
                line.currentIndexChanged["int"].connect(
                    partial(self.combobox_changed, line, j, "player_settings")
                )
            elif isinstance(line, QtWidgets.QLineEdit):
                line.returnPressed.connect(
                    partial(self.line_entered, line, j, "player_settings")
                )
    ...
    def save_config_settings(self):
        ...
        if (
            ui.player_val == "libmpv"
            and hasattr(ui.tab_5, "mpv")
    ...
    def basic_params(self, player=None, mode=None):
        ...
        if not ui.video_outputs or not ui.audio_outputs:
            ...
            change_opt_file(
                self.option_file,
                "AUDIO_OUTPUTS=",
                "AUDIO_OUTPUTS={}".format(ui.audio_outputs),
            )
            change_opt_file(
                self.option_file,
                "VIDEO_OUTPUTS=",
                "VIDEO_OUTPUTS={}".format(ui.video_outputs),
            )
        if not mpv_cmd:
            mpv_cmd = [
        ...
        for value in ["--vo", "--ao"]:
            ...
            if output:
                ...
                for i in output_list:
                    ...
                    if (
                        i
                        and not i.lower().startswith("available")
    ...
    def set_folder(self, widget, var_name=None):
        fname = QtWidgets.QFileDialog.getExistingDirectory(
            MainWindow, "Add Directory", ui.last_dir
        )
        if fname:
    ...
    def line_entered(self, widget, var_name=None, option=None):
        ...
        if var_name == "local_ip_stream":
            ...
            for extra_player in extra_players:
                if (
                    extra_player not in ui.playback_engine

...
class Browser(QtWebKitWidgets.QWebView):
    ...
    def contextMenuEvent(self, event):
        ...
        title_found = False
        self.ui.logger.info(
            "elm: {}; text:{};".format(hit.element(), hit.element().toPlainText())
        )
        try:
            if self.title_page:
                if "youtube.com" in self.url().toString():
        ...
        if not url.isEmpty() or self.img_url:
            if "tvdb" in url.toString():
                ...
                "youtube.com" in url.toString()
                or "ytimg.com" in url.toString()
                or url.toString().startswith("http")
            ):
                yt = True
                ...
                for i, acts in enumerate(item_m):
                    if act == acts:
                        if "views" in self.title_page:
                            self.title_page = re.sub(
                                "[0-9][^ ]* ", "", self.title_page, 1
                            )
                            self.title_page = re.sub(
                                "[0-9][^ ]* views", "", self.title_page, 1
                            )
                            self.title_page = self.title_page.replace("/", "-")
                        if not self.title_page:

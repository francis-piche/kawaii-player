...
class YTDL:
    def __init__(self, ui):
        ...
        if os.name == "posix":
            ...
            verify = False
        self.vnt = Vinanti(
            block=True, hdrs={"User-Agent": self.ui.user_agent}, verify=verify
        )
    ...
    def get_final_for_resolution(
        ...
        if os.name == "posix":
            ...
            content = subprocess.check_output(
                ytdl_list, stderr=subprocess.STDOUT, shell=True
            )
        ...
        if (
            sub_arr and self.ui.mpvplayer_val.processId() == 0
        ) or not self.ui.gapless_network_stream:
